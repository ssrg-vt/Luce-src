...
update_reg r e = do
  ...
  put (SState {regs = M.insert rr (simplify_expr c $ write_rreg r e' e) rs, mem = m, flags = fs, config = c, block_data = bd, regions = rios'})
...
update_mem a' s e' = do
  ...
  put (SState {regs = rs, mem = m', flags = fs,config = c, block_data = bd, regions = rios'})
...
update_flg f e' = do
  ...
  let e = simplify_expr c e'
  let rios' = Set.union rios $ get_memory_regions_expr e
  put (SState {regs = rs, mem = m, flags = M.insert f e fs,config = c, block_data = bd, regions = rios'})
...
update_flgs flgs e' = do
  SState {regs = rs, mem = m, flags = fs,config = c, block_data = bd, regions = rios} <- get
  let e = simplify_expr c e'
  puts flgs (\f (SState {regs = rs, mem = m, flags = fs,config = c, block_data = bd, regions = rios}) ->
                   SState {regs = rs, mem = m, flags = M.insert f e fs,config = c, block_data = bd, regions = rios})
...
read_mem e' s = do
  ...
  case M.size eq_blocks of
    ...
    0 -> case M.size overlapped of
           0 -> do
             put (SState {regs = rs, mem = M.insert (a,s) (E_deref a s) m, flags = fs,config = c, block_data = bd, regions = rios'})
             return $ E_deref a s
           _ -> do
             ...
             put (SState {regs = rs, mem = M.insert (a',s') e' not_overlapped, flags = fs,config = c, block_data = bd, regions = rios'})
...
get_op_size op = error $ "Cannot determine size of operand: " ++ show_operand (Just op)

-- lookup the signature of function name f in the config c
-- A signature provides the parameters to the function (e.g., RDI, RSI)
-- and where its return value is stored (e.g., RAX or XMM0)
lookup_signature c f =
  case M.lookup f (signatures c) of
    Nothing -> error $ "Cannot find function signature of " ++ f ++ " in config file:\n" ++ show_config c
    Just x -> x
...
-- PUSH
se_instr (Instr _ _ PUSH (Just op) _ _ _ _) = do
  let size = get_op_size op
  let e = E_app (Op SUB) [E_reg RSP, E_val (fromIntegral size) 64]
  e' <- se_expr e
  update_reg RSP e'
  e'' <- se_op op size
  update_mem e' size e''
-- POP
se_instr (Instr _ _ POP (Just (Reg r)) _ _ _ _) = do
  ...
  update_reg RSP e'
-- LEAVE
se_instr (Instr x0 x1 LEAVE x2 x3 x4 x5 x6) = do
  e' <- se_expr (E_reg RBP)
  update_reg RSP e'
  se_instr (Instr x0 x1 POP (Just (Reg RBP)) x3 x4 x5 x6)
-- RET
se_instr (Instr _ _ RET _ _ _ _ _) = do
  e'' <- se_op (Address (SizeDir 64 (FromReg RSP))) 8
  update_reg RIP e''
  e' <- se_expr (E_app (Op ADD) [E_reg RSP, E_val 8 64])
  update_reg RSP e'
-- CALL
se_instr (Instr addr _ CALL _ _ _ (Just (f,_)) _) = do
  -- Push RIP
  let e = E_app (Op SUB) [E_reg RSP, E_val 8 64]
  e' <- se_expr e
  update_reg RSP e'
  e'' <- se_expr (E_reg RIP)
  update_mem e' 8 e''
  {-
    As we do not have a corresponding RET to execute, the symbolic execution
    engine will not know to restore it, so we have to do that here and then
    manually add a sub in the call postcondition in Isa_EmbedBinary.
  -}
  e''' <- se_expr (E_app (Op ADD) [E_reg RSP, E_val 8 64])
  update_reg RSP e'''
  SState {regs = _, mem = _, flags = _, config = c, block_data = _, regions = _} <- get
  -- Go to starting point of called function
  update_reg RIP (lookup_address c f)
  let (_, args, dest) = lookup_signature c f
  es <- mapM (se_expr . E_reg) args
  update_reg dest (E_app (F2 f $ w64 addr) es)
-- XCHG
se_instr (Instr _ _ XCHG (Just op1) (Just op2) _ _ _) = do
  e1 <- se_op op1 (get_op_size op1)
  e2 <- se_op op2 (get_op_size op1)
  update_op op1 e2
  update_op op2 e1
-- MOV
se_instr (Instr _ _ MOV (Just op1) (Just op2) _ _ _) = do
  e2 <- se_op op2 (get_op_size op1)
  update_op op1 e2
-- MOVZX
se_instr (Instr _ _ MOVZX (Just op1) (Just op2) _ _ _) = do
  e2 <- se_op op2 (get_op_size op1)
  let size = get_op_size op2
  update_op op1 (E_app (TakeBits (size * 8 - 1) 0) [e2])
-- MOVSD
se_instr (Instr _ _ MOVSD (Just op1) (Just op2) _ _ _) = do
  e2 <- se_op op2 (get_op_size op1)
  update_op op1 (E_app (TakeBits 63 0) [e2])
-- MOVSS
se_instr (Instr _ _ MOVSS (Just op1) (Just op2) _ _ _) = do
  e2 <- se_op op2 (get_op_size op1)
  update_op op1 (E_app (TakeBits 31 0) [e2])
-- MOVAPD
se_instr (Instr _ _ MOVAPD (Just op1) (Just op2) _ _ _) = do
  e2 <- se_op op2 (get_op_size op1)
  update_op op1 e2
-- MOVDQA
se_instr (Instr _ _ MOVDQA (Just op1) (Just op2) _ _ _) = do
  e2 <- se_op op2 (get_op_size op1)
  update_op op1 e2
-- MOVDQU
se_instr (Instr _ _ MOVDQU (Just op1) (Just op2) _ _ _) = do
  e2 <- se_op op2 (get_op_size op1)
  update_op op1 e2
-- MOVUPD
se_instr (Instr _ _ MOVUPD (Just op1) (Just op2) _ _ _) = do
  e2 <- se_op op2 (get_op_size op1)
  update_op op1 e2
-- MOVABS
se_instr (Instr _ _ MOVABS (Just op1) (Just op2) _ _ _) = do
  e2 <- se_op op2 (get_op_size op1)
  update_op op1 e2
-- MOVAPS
se_instr (Instr _ _ MOVAPS (Just op1) (Just op2) _ _ _) = do
  e2 <- se_op op2 (get_op_size op1)
  update_op op1 e2
-- MOVUPS
se_instr (Instr _ _ MOVUPS (Just op1) (Just op2) _ _ _) = do
  e2 <- se_op op2 (get_op_size op1)
  update_op op1 e2
-- MOVQ
se_instr (Instr _ _ MOVQ (Just op1) (Just op2) _ _ _) = do
  e2 <- se_op op2 (get_op_size op1)
  update_op op1 (E_app (TakeBits 63 0) [e2])
-- MOVSX
se_instr (Instr _ _ MOVSX (Just op1) (Just op2) _ _ _) = do
  e2 <- se_op op2 (get_op_size op1)
  let s1 = get_op_size op1
  let s2 = get_op_size op2
  update_op op1 (E_app (SExtend (8*s2) (8*s1)) [e2])
-- MOVSXD
se_instr (Instr _ _ MOVSXD (Just op1) (Just op2) _ _ _) = do
  e2 <- se_op op2 (get_op_size op1)
  let s1 = get_op_size op1
  let s2 = get_op_size op2
  update_op op1 (E_app (SExtend (8*s2) (8*s1)) [e2])
-- CMOVA
se_instr (Instr _ _ CMOVA (Just op1) (Just op2) _ _ _) = do
  ss <- get
...
-- CMOVAE
se_instr (Instr _ _ CMOVAE (Just op1) (Just op2) _ _ _) = do
  ss <- get
  let f = mk_flg "!CF" ss
  let s = get_op_size op1
  e1 <- se_op op1 s
  ...
  update_op op1 (E_ite f e2 e1)
-- CMOVB
se_instr (Instr _ _ CMOVB (Just op1) (Just op2) _ _ _) = do
  ss <- get
  let f = mk_flg "CF" ss
  let s = get_op_size op1
  e1 <- se_op op1 s
  ...
  update_op op1 (E_ite f e2 e1)
-- CMOVBE
se_instr (Instr _ _ CMOVBE (Just op1) (Just op2) _ _ _) = do
  ss <- get
  let f = mk_flg "CF || ZF" ss
  let s = get_op_size op1
  e1 <- se_op op1 s
  ...
  update_op op1 (E_ite f e2 e1)
-- CMOVE
se_instr (Instr _ _ CMOVE (Just op1) (Just op2) _ _ _) = do
  ss <- get
  let f = mk_flg "ZF" ss
  let s = get_op_size op1
  e1 <- se_op op1 s
  ...
  update_op op1 (E_ite f e2 e1)
-- CMOVG
se_instr (Instr _ _ CMOVG (Just op1) (Just op2) _ _ _) = do
  ss <- get
  ...
  update_op op1 (E_ite f e2 e1)
-- CMOVGE
se_instr (Instr _ _ CMOVGE (Just op1) (Just op2) _ _ _) = do
  ss <- get
  ...
  update_op op1 (E_ite f e2 e1)
-- CMOVL
se_instr (Instr _ _ CMOVL (Just op1) (Just op2) _ _ _) = do
  ss <- get
  let f = mk_flg "SF != OF" ss
  let s = get_op_size op1
  e1 <- se_op op1 s
  ...
  update_op op1 (E_ite f e2 e1)
-- CMOVLE
se_instr (Instr _ _ CMOVLE (Just op1) (Just op2) _ _ _) = do
  ss <- get
  let f = mk_flg "ZF || SF != OF" ss
  let s = get_op_size op1
  e1 <- se_op op1 s
  ...
  update_op op1 (E_ite f e2 e1)
-- CMOVNE
se_instr (Instr _ _ CMOVNE (Just op1) (Just op2) _ _ _) = do
  ss <- get
  let f = mk_flg "!ZF" ss
  let s = get_op_size op1
  e1 <- se_op op1 s
  ...
  update_op op1 (E_ite f e2 e1)
-- CMOVNS
se_instr (Instr _ _ CMOVNS (Just op1) (Just op2) _ _ _) = do
  ss <- get
  let f = mk_flg "!SF" ss
  let s = get_op_size op1
  e1 <- se_op op1 s
  ...
  update_op op1 (E_ite f e2 e1)
-- CMOVS
se_instr (Instr _ _ CMOVS (Just op1) (Just op2) _ _ _) = do
  ss <- get
  let f = mk_flg "SF" ss
  let s = get_op_size op1
  e1 <- se_op op1 s
...
se_instr (Instr _ _ IMUL (Just op1) (Just op2) (Just op3) _ _) = do
  let size = get_op_size op1
  e2 <- se_op op2 size
  e3 <- se_op op3 size
  update_op op1 (E_app (TakeBits (size * 8 - 1) 0)  [E_app (Op IMUL) [e2,e3]])
-- MUL
se_instr (Instr _ _ MUL (Just op1) Nothing _ _ _) = do
  let size = get_op_size op1
  e2 <- se_op op1 size
  let (dst1,dst0) = case size of
                      8 -> (RDX,RAX)
...
se_instr (Instr _ _ MUL (Just op1) (Just op2) (Just op3) _ _) = do
  let size = get_op_size op1
  e2 <- se_op op2 size
  e3 <- se_op op3 size
  update_op op1 (E_app (TakeBits (size * 8 - 1) 0)  [E_app (Op MUL) [e2,e3]])
-- DIV
se_instr (Instr _ _ DIV (Just op1) Nothing _ _ _) = do
  let size = get_op_size op1
  -- dividend
...
-- IDIV
se_instr (Instr _ _ IDIV (Just op1) Nothing _ _ _) = do
  let size = get_op_size op1
  -- dividend
  let (h,l) = case size of
                8 -> (RDX,RAX)
...
-- SBB
se_instr (Instr _ _ SBB (Just op1) (Just op2) _ _ _) = do
  let size = get_op_size op1
  e2 <- se_op op2 size
  e1 <- se_op op1 size
  e_cf <- gets $ mk_flg "CF"
  ...
  update_flgs [ZF,CF,OF,SF,PF] e
-- BSR
se_instr (Instr _ _ BSR (Just op1) (Just op2) _ _ _) = do
  e2 <- se_op op2 (get_op_size op1)
  update_op op1 $ E_app (Op BSR) [e2]
-- BSWAP
se_instr (Instr _ _ BSWAP (Just op1) _ _ _ _) = do
  let size = get_op_size op1
  e1 <- se_op op1 size
  update_op op1 $ E_app (Op BSWAP) [E_app (F ZExtend) [e1, E_val (fromIntegral $ 8*size) (-1)]]
-- BT
se_instr (Instr _ _ BT (Just op1) (Just op2) _ _ _) = do
  let s = get_op_size op1
  base <- se_op op1 s
  offset <- se_op op2 s
...
-- BTR
se_instr (Instr _ _ BTR (Just op1) (Just op2) _ _ _) = do
  let s = get_op_size op1
  base <- se_op op1 s
  offset <- se_op op2 s
  let mod_offset = E_app (F Mod) [offset, E_val (w64 $ s * 8) 64]
...
-- BTS
se_instr (Instr _ _ BTS (Just op1) (Just op2) _ _ _) = do
  let s = get_op_size op1
  base <- se_op op1 s
  offset <- se_op op2 s
  let mod_offset = E_app (F Mod) [offset, E_val (w64 $ s * 8) 64]
...
-- UCOMISS
se_instr (Instr _ _ UCOMISS (Just op1) (Just op2) _ _ _) = do
  let size = get_op_size op1
  e2 <- se_op op2 size
  e1 <- se_op op1 size
...
-- UCOMISD
se_instr (Instr _ _ UCOMISD (Just op1) (Just op2) _ _ _) = do
  let size = get_op_size op1
  e2 <- se_op op2 size
  e1 <- se_op op1 size
  let e2_l = E_app (TakeBits 63 0) [e2]
  let e1_l = E_app (TakeBits 63 0) [e1]
  update_flgs [ZF,CF,OF,SF,PF] (E_app (Op UCOMISD) [e1_l,e2_l])
-- CVTSD2SS
se_instr (Instr _ _ CVTSD2SS (Just op1) (Just op2) _ _ _) = do
  let size = get_op_size op1
  e2 <- se_op op2 size
  let e2_l = E_app (TakeBits 63 0) [e2]
  update_op op1 (E_app (F FloatToSFloat) [e2_l])
-- CVTSS2SD
se_instr (Instr _ _ CVTSS2SD (Just op1) (Just op2) _ _ _) = do
  let size = get_op_size op1
  e2 <- se_op op2 size
  let e2_l = E_app (TakeBits 31 0) [e2]
  update_op op1 (E_app (F SFloatToFloat) [e2_l])
-- CVTSI2SD
se_instr (Instr _ _ CVTSI2SD (Just op1) (Just op2) _ _ _) = do
  let size = get_op_size op1
  e2 <- se_op op2 size
  e1 <- se_op op1 size
...
-- CVTTSD2SI
se_instr (Instr _ _ CVTTSD2SI (Just op1) (Just op2) _ _ _) = do
  let size = get_op_size op1
  e2 <- se_op op2 size
  let e2_l = E_app (TakeBits 63 0) [e2]
  update_op op1 (E_app (F FloatToInt) [e2_l]) -- TODO use size of destination
-- SETA
se_instr (Instr _ _ SETA (Just op1) _ _ _ _) = do
 ss <- get
 let e = mk_flg "!CF && !ZF" ss
 update_op op1 (E_ite e (E_val 1 1) (E_val 0 1))
-- SETB
se_instr (Instr _ _ SETB (Just op1) _ _ _ _) = do
 ss <- get
 let e = mk_flg "CF" ss
 update_op op1 (E_ite e (E_val 1 1) (E_val 0 1))
-- SETBE
se_instr (Instr _ _ SETBE (Just op1) _ _ _ _) = do
 ss <- get
 let e = mk_flg "CF || ZF" ss
 update_op op1 (E_ite e (E_val 1 1) (E_val 0 1))
-- SETE
se_instr (Instr _ _ SETE (Just op1) _ _ _ _) = do
 ss <- get
 let e = mk_flg "ZF" ss
 update_op op1 (E_ite e (E_val 1 1) (E_val 0 1))
-- SETG
se_instr (Instr _ _ SETG (Just op1) _ _ _ _) = do
 ss <- get
 let e = mk_flg "!ZF && SF == OF" ss
 update_op op1 (E_ite e (E_val 1 1) (E_val 0 1))
-- SETNE
se_instr (Instr _ _ SETNE (Just op1) _ _ _ _) = do
 ss <- get
 let e = mk_flg "!ZF" ss
 update_op op1 (E_ite e (E_val 1 1) (E_val 0 1))
-- SETAE
se_instr (Instr _ _ SETAE (Just op1) _ _ _ _) = do
 ss <- get
 let e = mk_flg "!CF" ss
 update_op op1 (E_ite e (E_val 1 1) (E_val 0 1))
-- SQRTSD
se_instr (Instr _ _ SQRTSD (Just op1) (Just op2) _ _ _) = do
  e2 <- se_op op2 (get_op_size op1)
  ...
  -- SIMD ARITHMETIC 64 BIT
    ...
    case (i_op1 i,i_op2 i) of
      (Just op1,Just op2) -> do
        ...
        update_op op1 (E_app Concat [e1_h, E_app f [e1_l,e2_l]])
  -- UNARY INSTRUCTIONS
  else if i_opcode i `elem` [NEG,NOT] then
    case i_op1 i of
      Just op1 -> do
        let size = get_op_size op1
        e1 <- se_op op1 size
...
se_instrs [] = return ()
se_instrs (i:is) = do
  update_reg RIP (E_val (fromIntegral $ i_addr i + i_size i) 64)
  se_instr i
  se_instrs is

initial_state :: Config -> [(Int,Instr)] -> SState
initial_state c bd = SState { regs = M.empty, mem = M.empty,flags = M.empty,config = c, block_data = bd, regions = Set.empty }
...
se_acode_function :: Config -> (Int -> Instr) -> _ -> ACode Int String -> ACode SState Expr

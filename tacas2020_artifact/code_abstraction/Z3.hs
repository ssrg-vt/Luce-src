...
z3_mk_expr c var_asts (E_app (Op SHL) [e0,E_val n _]) = do
  ...
  _2pn <- mkInteger (2^n)
  mkMul [ast0,_2pn]
z3_mk_expr c var_asts (E_app (Op AND) [e0,E_val 63 _]) = do -- TODO generalize
  ast0 <- z3_mk_expr c var_asts e0
  _64 <- mkInteger 64
  mkMod ast0 _64
...
z3_mk_expr c var_asts (E_app (F2 f a) _) = return $ fromJust $ M.lookup (E_var ("f" ++ showH a) (Known 0)) var_asts -- treat function results as free variables
z3_mk_expr c var_asts (E_app (SExtend _ 64) [e]) = z3_mk_expr c var_asts e -- values translated to ints remain their value after sign-extension
...
z3_mk_expr c var_asts (E_app (TakeBits 7 0) [e]) = do
  ...
  _256 <- mkInteger 256
  mkMod ast0 _256
-- z3_mk_expr c var_asts (E_app (TakeBits 15 8) [e]) = z3_mk_expr c var_asts $ E_app (TakeBits 7 0) [E_app (Op SHR) [e, E_val 8 (-1)]]

...
lemma take_bits_bad_arguments:
  assumes \<open>h < l\<close>
  shows \<open>\<langle>h,l\<rangle>w = 0\<close>
  using assms
...
lemma take_bits_bitXOR:
  fixes a b :: "'a::len0 word"
  assumes "h < LENGTH('a)"
  shows "((\<langle>h,l\<rangle>(a XOR b)) :: 'b::len0 word) = ((\<langle>h,l\<rangle>a) :: 'b::len0 word) XOR \<langle>h,l\<rangle>b"
  apply (rule word_eqI)
  using assms
  by (auto simp add: word_size bitXOR_nth test_bit_of_take_bits)


lemma take_bits_bitAND_bit32[simp]:
  fixes a b :: "'a::len0 word"
  assumes "LENGTH('b) \<ge> 32"
      and "LENGTH('a) \<ge> 32"
...
lemma take_minus_1:
  assumes "take n x = y"
      and "n > 0"
    shows "take (n-1) x = take (n-1) y"
  using assms
  by auto

lemma take_foldr:
  assumes "n \<le> length as"
      and "\<forall> a bs. take n (g a bs) = g' a (take n bs)"
      and "\<forall> n a bs . take n (g' a (take (Suc n) bs)) = g' a (take n bs)"
    shows "take n (foldr (\<lambda>x sm. f x # g x sm) as []) = foldr (\<lambda>x sm. f x # g' x sm) (take n as) [] "
 using assms
...
next
  case (Suc n)
  have "\<forall>a bs. take n (g a bs) = g' a (take n bs)"
  proof-
    {
      fix a bs
      have 1: "take (Suc n) (g a bs) = g' a (take (Suc n) bs)"
        using Suc(3)
        by auto
      hence "take n (g a bs) = g' a (take n bs)"
        using take_minus_1[OF 1] Suc(4)
        by auto
    }
    thus ?thesis
      by auto
  ...
  thus ?case
    using Suc(2-)
    using Suc(1)[of "tl as"]
    by (cases as;auto)
qed


lemma take_zip_smaller:
  assumes "n \<le> m"
  shows "take n (zip xs ys) = zip (take m xs) (take n ys)"
  using assms
proof(induct m arbitrary: n xs ys)
  case 0
  thus ?case
    by auto
next
  case (Suc m)
  show ?case
    using Suc(1)[of "n-1" "tl xs" "tl ys"] Suc(2-)
    by (cases n;cases ys;cases xs;auto)
qed

lemma rbl_plus_take_larger:
  shows "n \<le> m \<Longrightarrow> rbl_plus cin (map ((\<and>) ab) (take n as)) (take n bs) = rbl_plus cin (map ((\<and>) ab) (take m as)) (take n bs)"
proof(induct m arbitrary: cin ab n as bs)
  case 0
  thus ?case
    by auto
next
  case (Suc m)
  show ?case
    using Suc(2) Suc(1)[of n cin ab as bs]
    apply (cases "n=m+1",auto simp add: rbl_plus_def take_zip[symmetric])
    apply (subst take_map[symmetric])+
    apply (subst take_zip_smaller[symmetric,where m="Suc m"])
    apply auto
    apply (subst take_zip_smaller[symmetric,where m="m"])
    by (auto simp add: take_zip)
qed

lemma rbl_plus_cong_3:
  assumes "ys = ys'"
  shows "rbl_plus cin xs ys = rbl_plus cin xs ys'"
  using assms
  by auto

lemma rbl_plus_Cons:
  assumes "y \<noteq> []"
  shows "rbl_plus cin (x#xs) y = (xor3 x (hd y) cin)#rbl_plus (carry x (hd y) cin) xs (tl y)"
  using assms
  by (cases y;auto simp add: rbl_plus_def)

lemma take_rbl_mul:
  assumes "length a = length b"
      and "n \<le> length a"
  shows "take n (rbl_mul a b) = rbl_mul (take n a) (take n b)"
  using assms
proof(induct n arbitrary: a b)
  case 0
  thus ?case
    by (auto simp add: rbl_mul_def)
next
  case (Suc n)
  show ?case
    using Suc(2-)
    apply (cases a;cases b;auto simp add: rbl_mul_def take_rbl_plus take_map rbl_plus_Cons)
    apply (rule rbl_plus_cong_3)
    apply (rule take_foldr)
    apply simp
    apply (auto simp add: take_rbl_plus take_map)[1]
    by (auto simp add: take_rbl_plus take_map min_def rbl_plus_take_larger)[1]
qed

lemma take_bits_mul[simp]:
  fixes a b :: "'a::len word"
  assumes "LENGTH('a) \<ge> LENGTH('b)"
      and "h > LENGTH('b)"
  shows "((\<langle>h,0\<rangle>(a*b)) :: 'b ::len word) = \<langle>h,0\<rangle>a * \<langle>h,0\<rangle>b"
proof-
  have "to_bl ((\<langle>h,0\<rangle>(a*b)) :: 'b::len word) = to_bl (((\<langle>h,0\<rangle>a)::'b::len word) * \<langle>h,0\<rangle>b)"
    using assms
    unfolding take_bits_def
    ...
    apply (subst rbl_word_mul)
    apply (auto simp add: take_rbl_mul take_rev)
    apply (subst rbl_word_mul)
    by (auto simp add: word_rep_drop)
  thus ?thesis
    using word_bl.Rep_eqD
    by blast
qed

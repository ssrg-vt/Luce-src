...
theory Chum_Rewriting
  imports
      artifact_rewriting.Memory_Rewriting
      Chum_Execution
      BitVector_Rewriting
begin
...
lemma sub_overflow_flag_case1:
  fixes a b n :: nat
  shows "b < 2 ^ (n - Suc 0) \<Longrightarrow> a < b \<Longrightarrow> a + 2 ^ n - b < 2 ^ (n - Suc 0) \<Longrightarrow> False"
  by (induct n,auto)

lemma sub_overflow_flag_case2:
  fixes a b n :: nat
  shows "2 ^ (n - Suc 0) \<le> b \<Longrightarrow> b \<le> a \<Longrightarrow> 2 ^ (n - Suc 0) \<le> a - b \<Longrightarrow> a < 2 ^ n \<Longrightarrow> False"
  by (induct n,auto)
...
lemma sf_eq_of_iff_le:
  fixes a b :: "'a::len word"
  shows "(sub_sign_flag a b = sub_overflow_flag a b) \<longleftrightarrow> b <=s a"
proof-
  have 1: "unat (2 ^ (LENGTH('a) - Suc 0)::'a word) = 2 ^ (LENGTH('a) - Suc 0)"
    by (simp add: unat_2px)
  show ?thesis
    apply (auto simp add: 1 sub_sign_flag_def sub_overflow_flag_def msb_is_gt_2p word_sle_msb_le word_sless_msb_less)
    apply unat_arith
    using unat_lt2p[of a] unat_lt2p[of b] sub_overflow_flag_case1[of "unat b" "LENGTH('a)"  "unat a"]
    apply (auto simp add: 1 simp del: unat_lt2p)[1]
    apply unat_arith
    using unat_lt2p[of a] unat_lt2p[of b] sub_overflow_flag_case1[of "unat b" "LENGTH('a)"  "unat a"]
    apply (auto simp add: 1 simp del: unat_lt2p)[1]
    apply unat_arith
    apply unat_arith
    apply unat_arith
    using unat_lt2p[of a] unat_lt2p[of b] 
    apply (auto simp add: 1 simp del: unat_lt2p)[1]
    apply unat_arith
    using unat_lt2p[of a] unat_lt2p[of b]
    apply (auto simp add: 1 simp del: unat_lt2p)[1]
    ...
    apply unat_arith
    apply unat_arith
    using unat_lt2p[of a] unat_lt2p[of b] sub_overflow_flag_case2[of "LENGTH('a)" "unat b" "unat a"]
    apply (auto simp add: 1 simp del: unat_lt2p)[1]
    apply unat_arith
    using unat_lt2p[of a] unat_lt2p[of b] sub_overflow_flag_case2[of "LENGTH('a)" "unat b" "unat a"]
    apply (auto simp add: 1 simp del: unat_lt2p)[1]
...
lemma sf_neq_of_iff_le:
  fixes a b :: "'a::len word"
  shows "(sub_sign_flag a b = (\<not>sub_overflow_flag a b)) \<longleftrightarrow> \<not>b <=s a"
  using sf_eq_of_iff_le[of a b]
  by auto

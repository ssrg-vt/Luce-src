...
lemma take_bytes_of[simp]:
  assumes "h \<ge> l"
      and "LENGTH('a) div 8 > h"
  assumes "h + 2 \<le> n + LENGTH('a) div 8"
  fixes w :: "'a::len0 word"
  shows "take n (\<lbrace>h,l\<rbrace>w) = (if LENGTH('a) div 8 = Suc 0 \<and> h = 0 then \<lbrace>h,l\<rbrace>w else if n \<le> Suc (Suc (h + (LENGTH('a) div 8 - Suc l))) - LENGTH('a) div 8 then \<lbrace>h,h+1-n\<rbrace>w else \<lbrace>h,l\<rbrace>w)"
  using assms
  by (auto simp add: take_bytes_of')

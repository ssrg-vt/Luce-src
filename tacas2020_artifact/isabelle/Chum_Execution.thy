...
imports artifact_manual_execution.Manual_Execution
begin
...
primrec BV_unary_operate :: "BV_unary_operators \<Rightarrow> bv \<Rightarrow> bv"
  ...
  | "BV_unary_operate BV_Cvt_Int32_To_Double  bv = (let (w,s) = bv in ucast (cvtsi2sd (\<langle>31,0\<rangle> w)), 64)" (* 32 \<rightarrow> 64 bit*)
...
fun bv_read_memory' :: "assembly \<Rightarrow> state \<Rightarrow> nat \<Rightarrow> address \<Rightarrow> bv"
  ...
  | "bv_read_memory' \<alpha> \<sigma> s (A_Plus a0 a1) =
      (let' x0 = resolve_address \<alpha> \<sigma> a0;
            x1 = resolve_address \<alpha> \<sigma> a1 in
        (\<sigma> \<turnstile> *[x0 + x1,(s div 8)], s))"
  | "bv_read_memory' \<alpha> \<sigma> s (A_Mult a0 a1) =
      (let' x1 = resolve_address \<alpha> \<sigma> a1 in
        (\<sigma> \<turnstile> *[of_int a0 * x1,(s div 8)], s))"
  ...
  | "bv_read_memory' \<alpha> \<sigma> s (A_WordConstant a) =
      (\<sigma> \<turnstile> *[a,(s div 8)],s)"
  | "bv_read_memory' \<alpha> \<sigma> s (A_SizeDirective _ a) =
      bv_read_memory' \<alpha> \<sigma> s a"

fun bv_read_memory :: "assembly \<Rightarrow> state \<Rightarrow> nat \<Rightarrow> address \<Rightarrow> bv"
  where "bv_read_memory \<alpha> \<sigma> s a =
    (if s = 128 then 
        bv_cat (bv_read_memory' \<alpha> \<sigma> 64 (A_Plus a (A_WordConstant 8))) (bv_read_memory' \<alpha> \<sigma> 64 a)
     else if s \<le> 64 then
        bv_read_memory' \<alpha> \<sigma> s a
     else
         undefined)"
...
definition exec_instr :: "assembly \<Rightarrow> semantics \<Rightarrow> flag_annotation \<Rightarrow> instr \<Rightarrow> nat \<Rightarrow> state \<Rightarrow> state" where
  "exec_instr \<alpha> \<Psi> \<phi> i si \<sigma> \<equiv> let'
    flgs = filter_flag_annotation \<phi> (regs \<sigma> rip) [flag_pf,flag_cf,flag_zf,flag_sf,flag_of];
    \<sigma>' = incr_rip si \<sigma>;
    t = get_semantics \<alpha> \<Psi> i si;
    t' = remove_flags flgs t
  in t' \<sigma>'"

...
*)
theory bdrv_get_backing_filename
...
locale "bdrv_get_backing_filename" = "qemu_img_xen" +
  fixes RAX\<^sub>0\<^sub>v RCX\<^sub>0\<^sub>v RDX\<^sub>0\<^sub>v RDI\<^sub>0\<^sub>v RSI\<^sub>0\<^sub>v RSP\<^sub>0\<^sub>v ret_address\<^sub>v :: \<open>64 word\<close>
begin
...
definition P_0x19aa0_0 :: state_pred where
  \<open>P_0x19aa0_0 \<sigma> \<equiv> RIP \<sigma> = 0x19aa0 \<and> RAX \<sigma> = RAX\<^sub>0 \<and> RCX \<sigma> = RCX\<^sub>0 \<and> RDX \<sigma> = RDX\<^sub>0 \<and> RDI \<sigma> = RDI\<^sub>0 \<and> RSI \<sigma> = RSI\<^sub>0 \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address\<close>
declare P_0x19aa0_0_def[Ps]

definition P_0x19aa0_0_regions_set :: \<open>state \<Rightarrow> (nat \<times> 64 word \<times> nat) set\<close> where
  \<open>P_0x19aa0_0_regions_set \<sigma> \<equiv> {
    (0, RSP\<^sub>0, 8),
    (1, ((RDI\<^sub>0::64 word) + 0x850), 8)
  }\<close>

definition P_0x19aa0_0_regions :: state_pred where
  \<open>P_0x19aa0_0_regions \<sigma> \<equiv> \<exists>regions. P_0x19aa0_0_regions_set \<sigma> \<subseteq> regions
  \<and> (\<forall>i r. (i, r) \<in> regions \<longrightarrow> no_block_overflow r)
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<sqsubseteq> r' = (i = i' \<or> (i, i') \<in> {}))
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<bowtie> r' = (i \<noteq> i' \<and> (i, i') \<notin> {}))
\<close>

definition Q_0x19aae_0 :: state_pred where
  \<open>Q_0x19aae_0 \<sigma> \<equiv> RIP \<sigma> = 0x19aae \<and> RAX \<sigma> = RSI\<^sub>0 \<and> RCX \<sigma> = RCX\<^sub>0 \<and> RDX \<sigma> = RDX\<^sub>0 \<and> RDI \<sigma> = RDI\<^sub>0 \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare Q_0x19aae_0_def[Qs]

schematic_goal bdrv_get_backing_filename_0_3_0x19aa0_0x19aab_0[blocks]:
  assumes \<open>(P_0x19aa0_0 && P_0x19aa0_0_regions) \<sigma>\<close>
  shows \<open>exec_block 3 0x19aab 0 \<sigma> \<triangleq> ?\<sigma> \<and> Q_0x19aae_0 ?\<sigma> \<and> block_usage P_0x19aa0_0_regions_set \<sigma> ?\<sigma>\<close>
  using assms
  by (steps pre: P_0x19aa0_0_def P_0x19aa0_0_regions_def post: Q_0x19aae_0_def regionset: P_0x19aa0_0_regions_set_def)

definition P_0x19aae_true_1 :: state_pred where
  \<open>P_0x19aae_true_1 \<sigma> \<equiv> RIP \<sigma> = 0x19aae \<and> RAX \<sigma> = RSI\<^sub>0 \<and> RCX \<sigma> = RCX\<^sub>0 \<and> RDX \<sigma> = RDX\<^sub>0 \<and> RDI \<sigma> = RDI\<^sub>0 \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare P_0x19aae_true_1_def[Ps]

definition P_0x19aae_true_1_regions_set :: \<open>state \<Rightarrow> (nat \<times> 64 word \<times> nat) set\<close> where
  \<open>P_0x19aae_true_1_regions_set \<sigma> \<equiv> {
    (0, RSP\<^sub>0, 8),
    (1, ((RDI\<^sub>0::64 word) + 0x850), 8)
  }\<close>

definition P_0x19aae_true_1_regions :: state_pred where
  \<open>P_0x19aae_true_1_regions \<sigma> \<equiv> \<exists>regions. P_0x19aae_true_1_regions_set \<sigma> \<subseteq> regions
  \<and> (\<forall>i r. (i, r) \<in> regions \<longrightarrow> no_block_overflow r)
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<sqsubseteq> r' = (i = i' \<or> (i, i') \<in> {}))
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<bowtie> r' = (i \<noteq> i' \<and> (i, i') \<notin> {}))
\<close>

definition Q_0x5560_1 :: state_pred where
  \<open>Q_0x5560_1 \<sigma> \<equiv> RIP \<sigma> = 0x5560 \<and> RAX \<sigma> = RSI\<^sub>0 \<and> RCX \<sigma> = RCX\<^sub>0 \<and> RDI \<sigma> = RSI\<^sub>0 \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare Q_0x5560_1_def[Qs]

schematic_goal bdrv_get_backing_filename_0_4_0x19aae_0x19aca_1[blocks]:
  assumes \<open>(P_0x19aae_true_1 && P_0x19aae_true_1_regions && ZF) \<sigma>\<close>
  shows \<open>exec_block 4 0x19aca (Suc 0) \<sigma> \<triangleq> ?\<sigma> \<and> Q_0x5560_1 ?\<sigma> \<and> block_usage P_0x19aae_true_1_regions_set \<sigma> ?\<sigma>\<close>
  using assms
  by (steps pre: P_0x19aae_true_1_def P_0x19aae_true_1_regions_def post: Q_0x5560_1_def regionset: P_0x19aae_true_1_regions_set_def)

definition P_0x19aae_false_2 :: state_pred where
  \<open>P_0x19aae_false_2 \<sigma> \<equiv> RIP \<sigma> = 0x19aae \<and> RAX \<sigma> = RSI\<^sub>0 \<and> RCX \<sigma> = RCX\<^sub>0 \<and> RDX \<sigma> = RDX\<^sub>0 \<and> RDI \<sigma> = RDI\<^sub>0 \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare P_0x19aae_false_2_def[Ps]

definition P_0x19aae_false_2_regions_set :: \<open>state \<Rightarrow> (nat \<times> 64 word \<times> nat) set\<close> where
  \<open>P_0x19aae_false_2_regions_set \<sigma> \<equiv> {
    (0, RSP\<^sub>0, 8),
    (1, ((RDI\<^sub>0::64 word) + 0x850), 8)
  }\<close>

definition P_0x19aae_false_2_regions :: state_pred where
  \<open>P_0x19aae_false_2_regions \<sigma> \<equiv> \<exists>regions. P_0x19aae_false_2_regions_set \<sigma> \<subseteq> regions
  \<and> (\<forall>i r. (i, r) \<in> regions \<longrightarrow> no_block_overflow r)
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<sqsubseteq> r' = (i = i' \<or> (i, i') \<in> {}))
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<bowtie> r' = (i \<noteq> i' \<and> (i, i') \<notin> {}))
\<close>

definition Q_0x5560_2 :: state_pred where
  \<open>Q_0x5560_2 \<sigma> \<equiv> RIP \<sigma> = 0x5560 \<and> RAX \<sigma> = RSI\<^sub>0 \<and> RCX \<sigma> = RCX\<^sub>0 \<and> RDI \<sigma> = RSI\<^sub>0 \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare Q_0x5560_2_def[Qs]

schematic_goal bdrv_get_backing_filename_0_4_0x19aae_0x19aba_2[blocks]:
  assumes \<open>(P_0x19aae_false_2 && P_0x19aae_false_2_regions && ! ZF) \<sigma>\<close>
  shows \<open>exec_block 4 0x19aba 2 \<sigma> \<triangleq> ?\<sigma> \<and> Q_0x5560_2 ?\<sigma> \<and> block_usage P_0x19aae_false_2_regions_set \<sigma> ?\<sigma>\<close>
  using assms
  by (steps pre: P_0x19aae_false_2_def P_0x19aae_false_2_regions_def post: Q_0x5560_2_def regionset: P_0x19aae_false_2_regions_set_def)

definition P_0x5560_3 :: state_pred where
  \<open>P_0x5560_3 \<sigma> \<equiv> RIP \<sigma> = 0x5560 \<and> RAX \<sigma> = RSI\<^sub>0 \<and> RCX \<sigma> = RCX\<^sub>0 \<and> RDI \<sigma> = RSI\<^sub>0 \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare P_0x5560_3_def[Ps]

definition P_0x5560_3_regions_set :: \<open>state \<Rightarrow> (nat \<times> 64 word \<times> nat) set\<close> where
  \<open>P_0x5560_3_regions_set \<sigma> \<equiv> {
    (0, RSP\<^sub>0, 8),
    (1, ((RDI\<^sub>0::64 word) + 0x850), 8)
  }\<close>

definition P_0x5560_3_regions :: state_pred where
  \<open>P_0x5560_3_regions \<sigma> \<equiv> \<exists>regions. P_0x5560_3_regions_set \<sigma> \<subseteq> regions
  \<and> (\<forall>i r. (i, r) \<in> regions \<longrightarrow> no_block_overflow r)
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<sqsubseteq> r' = (i = i' \<or> (i, i') \<in> {}))
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<bowtie> r' = (i \<noteq> i' \<and> (i, i') \<notin> {}))
\<close>

definition Q_0x5563_3 :: state_pred where
  \<open>Q_0x5563_3 \<sigma> \<equiv> RIP \<sigma> = 0x5563 \<and> RAX \<sigma> = RSI\<^sub>0 \<and> RCX \<sigma> = RCX\<^sub>0 \<and> RDI \<sigma> = RSI\<^sub>0 \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare Q_0x5563_3_def[Qs]

schematic_goal bdrv_get_backing_filename_0_1_0x5560_0x5560_3[blocks]:
  assumes \<open>(P_0x5560_3 && P_0x5560_3_regions) \<sigma>\<close>
  shows \<open>exec_block (Suc 0) 0x5560 3 \<sigma> \<triangleq> ?\<sigma> \<and> Q_0x5563_3 ?\<sigma> \<and> block_usage P_0x5560_3_regions_set \<sigma> ?\<sigma>\<close>
  using assms
  by (steps pre: P_0x5560_3_def P_0x5560_3_regions_def post: Q_0x5563_3_def regionset: P_0x5560_3_regions_set_def)

definition P_0x5563_true_4 :: state_pred where
  \<open>P_0x5563_true_4 \<sigma> \<equiv> RIP \<sigma> = 0x5563 \<and> RAX \<sigma> = RSI\<^sub>0 \<and> RCX \<sigma> = RCX\<^sub>0 \<and> RDI \<sigma> = RSI\<^sub>0 \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare P_0x5563_true_4_def[Ps]

definition P_0x5563_true_4_regions_set :: \<open>state \<Rightarrow> (nat \<times> 64 word \<times> nat) set\<close> where
  \<open>P_0x5563_true_4_regions_set \<sigma> \<equiv> {
    (0, RSP\<^sub>0, 8),
    (1, ((RDI\<^sub>0::64 word) + 0x850), 8)
  }\<close>

definition P_0x5563_true_4_regions :: state_pred where
  \<open>P_0x5563_true_4_regions \<sigma> \<equiv> \<exists>regions. P_0x5563_true_4_regions_set \<sigma> \<subseteq> regions
  \<and> (\<forall>i r. (i, r) \<in> regions \<longrightarrow> no_block_overflow r)
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<sqsubseteq> r' = (i = i' \<or> (i, i') \<in> {}))
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<bowtie> r' = (i \<noteq> i' \<and> (i, i') \<notin> {}))
\<close>

definition Q_0x559b_4 :: state_pred where
  \<open>Q_0x559b_4 \<sigma> \<equiv> RIP \<sigma> = 0x559b \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare Q_0x559b_4_def[Qs]

schematic_goal bdrv_get_backing_filename_0_1_0x5563_0x5563_4[blocks]:
  assumes \<open>(P_0x5563_true_4 && P_0x5563_true_4_regions && ZF) \<sigma>\<close>
  shows \<open>exec_block (Suc 0) 0x5563 4 \<sigma> \<triangleq> ?\<sigma> \<and> Q_0x559b_4 ?\<sigma> \<and> block_usage P_0x5563_true_4_regions_set \<sigma> ?\<sigma>\<close>
  using assms
  by (steps pre: P_0x5563_true_4_def P_0x5563_true_4_regions_def post: Q_0x559b_4_def regionset: P_0x5563_true_4_regions_set_def)

definition P_0x5563_false_5 :: state_pred where
  \<open>P_0x5563_false_5 \<sigma> \<equiv> RIP \<sigma> = 0x5563 \<and> RAX \<sigma> = RSI\<^sub>0 \<and> RCX \<sigma> = RCX\<^sub>0 \<and> RDI \<sigma> = RSI\<^sub>0 \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare P_0x5563_false_5_def[Ps]

definition P_0x5563_false_5_regions_set :: \<open>state \<Rightarrow> (nat \<times> 64 word \<times> nat) set\<close> where
  \<open>P_0x5563_false_5_regions_set \<sigma> \<equiv> {
    (0, RDX \<sigma>, Suc 0),
    (1, RSP\<^sub>0, 8),
    (2, ((RDI\<^sub>0::64 word) + 0x850), 8)
  }\<close>

definition P_0x5563_false_5_regions :: state_pred where
  \<open>P_0x5563_false_5_regions \<sigma> \<equiv> \<exists>regions. P_0x5563_false_5_regions_set \<sigma> \<subseteq> regions
  \<and> (\<forall>i r. (i, r) \<in> regions \<longrightarrow> no_block_overflow r)
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<sqsubseteq> r' = (i = i' \<or> (i, i') \<in> {}))
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<bowtie> r' = (i \<noteq> i' \<and> (i, i') \<notin> {}))
\<close>

definition Q_0x556e_5 :: state_pred where
  \<open>Q_0x556e_5 \<sigma> \<equiv> RIP \<sigma> = 0x556e \<and> RAX \<sigma> = ucast ((\<sigma> \<turnstile> *[RDX \<sigma>,1]::8 word)) \<and> RCX \<sigma> = ((RDX \<sigma>::64 word) + 0x1) \<and> RDI \<sigma> = RSI\<^sub>0 \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RDX \<sigma>,1]::8 word) = ucast ((\<sigma> \<turnstile> *[RDX \<sigma>,1]::8 word)) \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare Q_0x556e_5_def[Qs]

schematic_goal bdrv_get_backing_filename_0_4_0x5563_0x556c_5[blocks]:
  assumes \<open>(P_0x5563_false_5 && P_0x5563_false_5_regions && ! ZF) \<sigma>\<close>
  shows \<open>exec_block 4 0x556c 5 \<sigma> \<triangleq> ?\<sigma> \<and> Q_0x556e_5 ?\<sigma> \<and> block_usage P_0x5563_false_5_regions_set \<sigma> ?\<sigma>\<close>
  using assms
  by (steps pre: P_0x5563_false_5_def P_0x5563_false_5_regions_def post: Q_0x556e_5_def regionset: P_0x5563_false_5_regions_set_def)

definition P_0x556e_true_6 :: state_pred where
  \<open>P_0x556e_true_6 \<sigma> \<equiv> RIP \<sigma> = 0x556e \<and> RAX \<sigma> = ucast ((\<sigma> \<turnstile> *[RDX \<sigma>,1]::8 word)) \<and> RCX \<sigma> = ((RDX \<sigma>::64 word) + 0x1) \<and> RDI \<sigma> = RSI\<^sub>0 \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RDX \<sigma>,1]::8 word) = ucast ((\<sigma> \<turnstile> *[RDX \<sigma>,1]::8 word)) \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare P_0x556e_true_6_def[Ps]

definition P_0x556e_true_6_regions_set :: \<open>state \<Rightarrow> (nat \<times> 64 word \<times> nat) set\<close> where
  \<open>P_0x556e_true_6_regions_set \<sigma> \<equiv> {
    (0, RDX \<sigma>, Suc 0),
    (1, RSP\<^sub>0, 8),
    (2, ((RDI\<^sub>0::64 word) + 0x850), 8)
  }\<close>

definition P_0x556e_true_6_regions :: state_pred where
  \<open>P_0x556e_true_6_regions \<sigma> \<equiv> \<exists>regions. P_0x556e_true_6_regions_set \<sigma> \<subseteq> regions
  \<and> (\<forall>i r. (i, r) \<in> regions \<longrightarrow> no_block_overflow r)
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<sqsubseteq> r' = (i = i' \<or> (i, i') \<in> {}))
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<bowtie> r' = (i \<noteq> i' \<and> (i, i') \<notin> {}))
\<close>

definition Q_0x5598_6 :: state_pred where
  \<open>Q_0x5598_6 \<sigma> \<equiv> RIP \<sigma> = 0x5598 \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare Q_0x5598_6_def[Qs]

schematic_goal bdrv_get_backing_filename_0_1_0x556e_0x556e_6[blocks]:
  assumes \<open>(P_0x556e_true_6 && P_0x556e_true_6_regions && ZF) \<sigma>\<close>
  shows \<open>exec_block (Suc 0) 0x556e 6 \<sigma> \<triangleq> ?\<sigma> \<and> Q_0x5598_6 ?\<sigma> \<and> block_usage P_0x556e_true_6_regions_set \<sigma> ?\<sigma>\<close>
  using assms
  by (steps pre: P_0x556e_true_6_def P_0x556e_true_6_regions_def post: Q_0x5598_6_def regionset: P_0x556e_true_6_regions_set_def)

definition P_0x556e_false_7 :: state_pred where
  \<open>P_0x556e_false_7 \<sigma> \<equiv> RIP \<sigma> = 0x556e \<and> RAX \<sigma> = ucast ((\<sigma> \<turnstile> *[RDX \<sigma>,1]::8 word)) \<and> RCX \<sigma> = ((RDX \<sigma>::64 word) + 0x1) \<and> RDI \<sigma> = RSI\<^sub>0 \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RDX \<sigma>,1]::8 word) = ucast ((\<sigma> \<turnstile> *[RDX \<sigma>,1]::8 word)) \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare P_0x556e_false_7_def[Ps]

definition P_0x556e_false_7_regions_set :: \<open>state \<Rightarrow> (nat \<times> 64 word \<times> nat) set\<close> where
  \<open>P_0x556e_false_7_regions_set \<sigma> \<equiv> {
    (0, RDX \<sigma>, Suc 0),
    (1, RSP\<^sub>0, 8),
    (2, ((RDI\<^sub>0::64 word) + 0x850), 8)
  }\<close>

definition P_0x556e_false_7_regions :: state_pred where
  \<open>P_0x556e_false_7_regions \<sigma> \<equiv> \<exists>regions. P_0x556e_false_7_regions_set \<sigma> \<subseteq> regions
  \<and> (\<forall>i r. (i, r) \<in> regions \<longrightarrow> no_block_overflow r)
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<sqsubseteq> r' = (i = i' \<or> (i, i') \<in> {}))
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<bowtie> r' = (i \<noteq> i' \<and> (i, i') \<notin> {}))
\<close>

definition Q_0x5578_7 :: state_pred where
  \<open>Q_0x5578_7 \<sigma> \<equiv> RIP \<sigma> = 0x5578 \<and> RDX \<sigma> = ((((RSI\<^sub>0::64 word) + (\<langle>63,0\<rangle>(((sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word)::64 word) * 0x1)::64 word))::64 word) - 0x1) \<and> RDI \<sigma> = RSI\<^sub>0 \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare Q_0x5578_7_def[Qs]

schematic_goal bdrv_get_backing_filename_0_3_0x556e_0x5575_7[blocks]:
  assumes \<open>(P_0x556e_false_7 && P_0x556e_false_7_regions && ! ZF) \<sigma>\<close>
  shows \<open>exec_block 3 0x5575 7 \<sigma> \<triangleq> ?\<sigma> \<and> Q_0x5578_7 ?\<sigma> \<and> block_usage P_0x556e_false_7_regions_set \<sigma> ?\<sigma>\<close>
  using assms
  by (steps pre: P_0x556e_false_7_def P_0x556e_false_7_regions_def post: Q_0x5578_7_def regionset: P_0x556e_false_7_regions_set_def)

definition P_0x5578_true_8 :: state_pred where
  \<open>P_0x5578_true_8 \<sigma> \<equiv> RIP \<sigma> = 0x5578 \<and> RDX \<sigma> = ((((RSI\<^sub>0::64 word) + (\<langle>63,0\<rangle>(((sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word)::64 word) * 0x1)::64 word))::64 word) - 0x1) \<and> RDI \<sigma> = RSI\<^sub>0 \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare P_0x5578_true_8_def[Ps]

definition P_0x5578_true_8_regions_set :: \<open>state \<Rightarrow> (nat \<times> 64 word \<times> nat) set\<close> where
  \<open>P_0x5578_true_8_regions_set \<sigma> \<equiv> {
    (0, RSP\<^sub>0, 8),
    (1, ((RDI\<^sub>0::64 word) + 0x850), 8)
  }\<close>

definition P_0x5578_true_8_regions :: state_pred where
  \<open>P_0x5578_true_8_regions \<sigma> \<equiv> \<exists>regions. P_0x5578_true_8_regions_set \<sigma> \<subseteq> regions
  \<and> (\<forall>i r. (i, r) \<in> regions \<longrightarrow> no_block_overflow r)
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<sqsubseteq> r' = (i = i' \<or> (i, i') \<in> {}))
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<bowtie> r' = (i \<noteq> i' \<and> (i, i') \<notin> {}))
\<close>

definition Q_0x5585_8 :: state_pred where
  \<open>Q_0x5585_8 \<sigma> \<equiv> RIP \<sigma> = 0x5585 \<and> RDX \<sigma> = ((((RSI\<^sub>0::64 word) + (\<langle>63,0\<rangle>(((sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word)::64 word) * 0x1)::64 word))::64 word) - 0x1) \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare Q_0x5585_8_def[Qs]

schematic_goal bdrv_get_backing_filename_0_1_0x5578_0x5578_8[blocks]:
  assumes \<open>(P_0x5578_true_8 && P_0x5578_true_8_regions && CF) \<sigma>\<close>
  shows \<open>exec_block (Suc 0) 0x5578 8 \<sigma> \<triangleq> ?\<sigma> \<and> Q_0x5585_8 ?\<sigma> \<and> block_usage P_0x5578_true_8_regions_set \<sigma> ?\<sigma>\<close>
  using assms
  by (steps pre: P_0x5578_true_8_def P_0x5578_true_8_regions_def post: Q_0x5585_8_def regionset: P_0x5578_true_8_regions_set_def)

definition P_0x5585_9 :: state_pred where
  \<open>P_0x5585_9 \<sigma> \<equiv> RIP \<sigma> = 0x5585 \<and> RDX \<sigma> = ((((RSI\<^sub>0::64 word) + (\<langle>63,0\<rangle>(((sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word)::64 word) * 0x1)::64 word))::64 word) - 0x1) \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare P_0x5585_9_def[Ps]

definition P_0x5585_9_regions_set :: \<open>state \<Rightarrow> (nat \<times> 64 word \<times> nat) set\<close> where
  \<open>P_0x5585_9_regions_set \<sigma> \<equiv> {
    (0, RCX \<sigma>, Suc 0),
    (1, RDI \<sigma>, Suc 0),
    (2, RSP\<^sub>0, 8),
...
definition P_0x5585_9_regions :: state_pred where
  \<open>P_0x5585_9_regions \<sigma> \<equiv> \<exists>regions. P_0x5585_9_regions_set \<sigma> \<subseteq> regions
  \<and> (\<forall>i r. (i, r) \<in> regions \<longrightarrow> no_block_overflow r)
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<sqsubseteq> r' = (i = i' \<or> (i, i') \<in> {}))
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<bowtie> r' = (i \<noteq> i' \<and> (i, i') \<notin> {}))
\<close>

definition Q_0x5596_9 :: state_pred where
  \<open>Q_0x5596_9 \<sigma> \<equiv> RIP \<sigma> = 0x5596 \<and> RDX \<sigma> = ((((RSI\<^sub>0::64 word) + (\<langle>63,0\<rangle>(((sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word)::64 word) * 0x1)::64 word))::64 word) - 0x1) \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare Q_0x5596_9_def[Qs]

schematic_goal bdrv_get_backing_filename_0_5_0x5585_0x5594_9[blocks]:
  assumes \<open>(P_0x5585_9 && P_0x5585_9_regions) \<sigma>\<close>
  shows \<open>exec_block 5 0x5594 9 \<sigma> \<triangleq> ?\<sigma> \<and> Q_0x5596_9 ?\<sigma> \<and> block_usage P_0x5585_9_regions_set \<sigma> ?\<sigma>\<close>
  using assms
  by (steps pre: P_0x5585_9_def P_0x5585_9_regions_def post: Q_0x5596_9_def regionset: P_0x5585_9_regions_set_def)

definition P_0x5596_true_10 :: state_pred where
  \<open>P_0x5596_true_10 \<sigma> \<equiv> RIP \<sigma> = 0x5596 \<and> RDX \<sigma> = ((((RSI\<^sub>0::64 word) + (\<langle>63,0\<rangle>(((sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word)::64 word) * 0x1)::64 word))::64 word) - 0x1) \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare P_0x5596_true_10_def[Ps]

definition P_0x5596_true_10_regions_set :: \<open>state \<Rightarrow> (nat \<times> 64 word \<times> nat) set\<close> where
  \<open>P_0x5596_true_10_regions_set \<sigma> \<equiv> {
    (0, RSP\<^sub>0, 8),
    (1, ((RDI\<^sub>0::64 word) + 0x850), 8)
  }\<close>

definition P_0x5596_true_10_regions :: state_pred where
  \<open>P_0x5596_true_10_regions \<sigma> \<equiv> \<exists>regions. P_0x5596_true_10_regions_set \<sigma> \<subseteq> regions
  \<and> (\<forall>i r. (i, r) \<in> regions \<longrightarrow> no_block_overflow r)
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<sqsubseteq> r' = (i = i' \<or> (i, i') \<in> {}))
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<bowtie> r' = (i \<noteq> i' \<and> (i, i') \<notin> {}))
\<close>

definition Q_0x5583_10 :: state_pred where
  \<open>Q_0x5583_10 \<sigma> \<equiv> RIP \<sigma> = 0x5583 \<and> RDX \<sigma> = ((((RSI\<^sub>0::64 word) + (\<langle>63,0\<rangle>(((sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word)::64 word) * 0x1)::64 word))::64 word) - 0x1) \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare Q_0x5583_10_def[Qs]

schematic_goal bdrv_get_backing_filename_0_2_0x5596_0x5580_10[blocks]:
  assumes \<open>(P_0x5596_true_10 && P_0x5596_true_10_regions && !ZF) \<sigma>\<close>
  shows \<open>exec_block 2 0x5580 10 \<sigma> \<triangleq> ?\<sigma> \<and> Q_0x5583_10 ?\<sigma> \<and> block_usage P_0x5596_true_10_regions_set \<sigma> ?\<sigma>\<close>
  using assms
  by (steps pre: P_0x5596_true_10_def P_0x5596_true_10_regions_def post: Q_0x5583_10_def regionset: P_0x5596_true_10_regions_set_def)

definition P_0x5583_true_11 :: state_pred where
  \<open>P_0x5583_true_11 \<sigma> \<equiv> RIP \<sigma> = 0x5583 \<and> RDX \<sigma> = ((((RSI\<^sub>0::64 word) + (\<langle>63,0\<rangle>(((sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word)::64 word) * 0x1)::64 word))::64 word) - 0x1) \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare P_0x5583_true_11_def[Ps]

definition P_0x5583_true_11_regions_set :: \<open>state \<Rightarrow> (nat \<times> 64 word \<times> nat) set\<close> where
  \<open>P_0x5583_true_11_regions_set \<sigma> \<equiv> {
    (0, RSP\<^sub>0, 8),
    (1, ((RDI\<^sub>0::64 word) + 0x850), 8)
  }\<close>

definition P_0x5583_true_11_regions :: state_pred where
  \<open>P_0x5583_true_11_regions \<sigma> \<equiv> \<exists>regions. P_0x5583_true_11_regions_set \<sigma> \<subseteq> regions
  \<and> (\<forall>i r. (i, r) \<in> regions \<longrightarrow> no_block_overflow r)
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<sqsubseteq> r' = (i = i' \<or> (i, i') \<in> {}))
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<bowtie> r' = (i \<noteq> i' \<and> (i, i') \<notin> {}))
\<close>
...
definition Q_0x5598_11 :: state_pred where
  \<open>Q_0x5598_11 \<sigma> \<equiv> RIP \<sigma> = 0x5598 \<and> RDX \<sigma> = ((((RSI\<^sub>0::64 word) + (\<langle>63,0\<rangle>(((sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word)::64 word) * 0x1)::64 word))::64 word) - 0x1) \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare Q_0x5598_11_def[Qs]

schematic_goal bdrv_get_backing_filename_0_1_0x5583_0x5583_11[blocks]:
  assumes \<open>(P_0x5583_true_11 && P_0x5583_true_11_regions && ZF) \<sigma>\<close>
  shows \<open>exec_block (Suc 0) 0x5583 11 \<sigma> \<triangleq> ?\<sigma> \<and> Q_0x5598_11 ?\<sigma> \<and> block_usage P_0x5583_true_11_regions_set \<sigma> ?\<sigma>\<close>
  using assms
  by (steps pre: P_0x5583_true_11_def P_0x5583_true_11_regions_def post: Q_0x5598_11_def regionset: P_0x5583_true_11_regions_set_def)

definition P_0x5583_false_12 :: state_pred where
  \<open>P_0x5583_false_12 \<sigma> \<equiv> RIP \<sigma> = 0x5583 \<and> RDX \<sigma> = ((((RSI\<^sub>0::64 word) + (\<langle>63,0\<rangle>(((sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word)::64 word) * 0x1)::64 word))::64 word) - 0x1) \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare P_0x5583_false_12_def[Ps]

definition P_0x5583_false_12_regions_set :: \<open>state \<Rightarrow> (nat \<times> 64 word \<times> nat) set\<close> where
  \<open>P_0x5583_false_12_regions_set \<sigma> \<equiv> {
    (0, RSP\<^sub>0, 8),
    (1, ((RDI\<^sub>0::64 word) + 0x850), 8)
  }\<close>

definition P_0x5583_false_12_regions :: state_pred where
  \<open>P_0x5583_false_12_regions \<sigma> \<equiv> \<exists>regions. P_0x5583_false_12_regions_set \<sigma> \<subseteq> regions
  \<and> (\<forall>i r. (i, r) \<in> regions \<longrightarrow> no_block_overflow r)
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<sqsubseteq> r' = (i = i' \<or> (i, i') \<in> {}))
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<bowtie> r' = (i \<noteq> i' \<and> (i, i') \<notin> {}))
\<close>

definition Q_0x5585_12 :: state_pred where
  \<open>Q_0x5585_12 \<sigma> \<equiv> RIP \<sigma> = 0x5585 \<and> RDX \<sigma> = ((((RSI\<^sub>0::64 word) + (\<langle>63,0\<rangle>(((sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word)::64 word) * 0x1)::64 word))::64 word) - 0x1) \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare Q_0x5585_12_def[Qs]

schematic_goal bdrv_get_backing_filename_0_1_0x5583_0x5583_12[blocks]:
  assumes \<open>(P_0x5583_false_12 && P_0x5583_false_12_regions && ! ZF) \<sigma>\<close>
  shows \<open>exec_block (Suc 0) 0x5583 12 \<sigma> \<triangleq> ?\<sigma> \<and> Q_0x5585_12 ?\<sigma> \<and> block_usage P_0x5583_false_12_regions_set \<sigma> ?\<sigma>\<close>
  using assms
  by (steps pre: P_0x5583_false_12_def P_0x5583_false_12_regions_def post: Q_0x5585_12_def regionset: P_0x5583_false_12_regions_set_def)

definition P_0x5596_false_13 :: state_pred where
  \<open>P_0x5596_false_13 \<sigma> \<equiv> RIP \<sigma> = 0x5596 \<and> RDX \<sigma> = ((((RSI\<^sub>0::64 word) + (\<langle>63,0\<rangle>(((sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word)::64 word) * 0x1)::64 word))::64 word) - 0x1) \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare P_0x5596_false_13_def[Ps]

definition P_0x5596_false_13_regions_set :: \<open>state \<Rightarrow> (nat \<times> 64 word \<times> nat) set\<close> where
  \<open>P_0x5596_false_13_regions_set \<sigma> \<equiv> {
    (0, RSP\<^sub>0, 8),
    (1, ((RDI\<^sub>0::64 word) + 0x850), 8)
  }\<close>

definition P_0x5596_false_13_regions :: state_pred where
  \<open>P_0x5596_false_13_regions \<sigma> \<equiv> \<exists>regions. P_0x5596_false_13_regions_set \<sigma> \<subseteq> regions
  \<and> (\<forall>i r. (i, r) \<in> regions \<longrightarrow> no_block_overflow r)
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<sqsubseteq> r' = (i = i' \<or> (i, i') \<in> {}))
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<bowtie> r' = (i \<noteq> i' \<and> (i, i') \<notin> {}))
\<close>
...
schematic_goal bdrv_get_backing_filename_0_1_0x5596_0x5596_13[blocks]:
  assumes \<open>(P_0x5596_false_13 && P_0x5596_false_13_regions && ! !ZF) \<sigma>\<close>
  shows \<open>exec_block (Suc 0) 0x5596 13 \<sigma> \<triangleq> ?\<sigma> \<and> Q_0x5598_13 ?\<sigma> \<and> block_usage P_0x5596_false_13_regions_set \<sigma> ?\<sigma>\<close>
  using assms
  by (steps pre: P_0x5596_false_13_def P_0x5596_false_13_regions_def post: Q_0x5598_13_def regionset: P_0x5596_false_13_regions_set_def)

definition P_0x5578_false_14 :: state_pred where
  \<open>P_0x5578_false_14 \<sigma> \<equiv> RIP \<sigma> = 0x5578 \<and> RDX \<sigma> = ((((RSI\<^sub>0::64 word) + (\<langle>63,0\<rangle>(((sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word)::64 word) * 0x1)::64 word))::64 word) - 0x1) \<and> RDI \<sigma> = RSI\<^sub>0 \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare P_0x5578_false_14_def[Ps]

definition P_0x5578_false_14_regions_set :: \<open>state \<Rightarrow> (nat \<times> 64 word \<times> nat) set\<close> where
  \<open>P_0x5578_false_14_regions_set \<sigma> \<equiv> {
    (0, RSP\<^sub>0, 8),
    (1, ((RDI\<^sub>0::64 word) + 0x850), 8)
  }\<close>

definition P_0x5578_false_14_regions :: state_pred where
  \<open>P_0x5578_false_14_regions \<sigma> \<equiv> \<exists>regions. P_0x5578_false_14_regions_set \<sigma> \<subseteq> regions
  \<and> (\<forall>i r. (i, r) \<in> regions \<longrightarrow> no_block_overflow r)
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<sqsubseteq> r' = (i = i' \<or> (i, i') \<in> {}))
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<bowtie> r' = (i \<noteq> i' \<and> (i, i') \<notin> {}))
\<close>

definition Q_0x5598_14 :: state_pred where
  \<open>Q_0x5598_14 \<sigma> \<equiv> RIP \<sigma> = 0x5598 \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare Q_0x5598_14_def[Qs]

schematic_goal bdrv_get_backing_filename_0_2_0x5578_0x557a_14[blocks]:
  assumes \<open>(P_0x5578_false_14 && P_0x5578_false_14_regions && ! CF) \<sigma>\<close>
  shows \<open>exec_block 2 0x557a 14 \<sigma> \<triangleq> ?\<sigma> \<and> Q_0x5598_14 ?\<sigma> \<and> block_usage P_0x5578_false_14_regions_set \<sigma> ?\<sigma>\<close>
  using assms
  by (steps pre: P_0x5578_false_14_def P_0x5578_false_14_regions_def post: Q_0x5598_14_def regionset: P_0x5578_false_14_regions_set_def)

definition P_0x5598_15 :: state_pred where
  \<open>P_0x5598_15 \<sigma> \<equiv> RIP \<sigma> = 0x5598 \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare P_0x5598_15_def[Ps]

definition P_0x5598_15_regions_set :: \<open>state \<Rightarrow> (nat \<times> 64 word \<times> nat) set\<close> where
  \<open>P_0x5598_15_regions_set \<sigma> \<equiv> {
    (0, RDI \<sigma>, Suc 0),
    (1, RSP\<^sub>0, 8),
    (2, ((RDI\<^sub>0::64 word) + 0x850), 8)
  }\<close>

definition P_0x5598_15_regions :: state_pred where
  \<open>P_0x5598_15_regions \<sigma> \<equiv> \<exists>regions. P_0x5598_15_regions_set \<sigma> \<subseteq> regions
  \<and> (\<forall>i r. (i, r) \<in> regions \<longrightarrow> no_block_overflow r)
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<sqsubseteq> r' = (i = i' \<or> (i, i') \<in> {}))
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<bowtie> r' = (i \<noteq> i' \<and> (i, i') \<notin> {}))
\<close>

definition Q_0x559b_15 :: state_pred where
  \<open>Q_0x559b_15 \<sigma> \<equiv> RIP \<sigma> = 0x559b \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare Q_0x559b_15_def[Qs]

schematic_goal bdrv_get_backing_filename_0_1_0x5598_0x5598_15[blocks]:
  assumes \<open>(P_0x5598_15 && P_0x5598_15_regions) \<sigma>\<close>
  shows \<open>exec_block (Suc 0) 0x5598 15 \<sigma> \<triangleq> ?\<sigma> \<and> Q_0x559b_15 ?\<sigma> \<and> block_usage P_0x5598_15_regions_set \<sigma> ?\<sigma>\<close>
  using assms
  by (steps pre: P_0x5598_15_def P_0x5598_15_regions_def post: Q_0x559b_15_def regionset: P_0x5598_15_regions_set_def)

definition P_0x559b_16 :: state_pred where
  \<open>P_0x559b_16 \<sigma> \<equiv> RIP \<sigma> = 0x559b \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = RSP\<^sub>0 \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare P_0x559b_16_def[Ps]

definition P_0x559b_16_regions_set :: \<open>state \<Rightarrow> (nat \<times> 64 word \<times> nat) set\<close> where
  \<open>P_0x559b_16_regions_set \<sigma> \<equiv> {
    (0, RSP\<^sub>0, 8),
    (1, ((RDI\<^sub>0::64 word) + 0x850), 8)
  }\<close>

definition P_0x559b_16_regions :: state_pred where
  \<open>P_0x559b_16_regions \<sigma> \<equiv> \<exists>regions. P_0x559b_16_regions_set \<sigma> \<subseteq> regions
  \<and> (\<forall>i r. (i, r) \<in> regions \<longrightarrow> no_block_overflow r)
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<sqsubseteq> r' = (i = i' \<or> (i, i') \<in> {}))
  \<and> (\<forall>i r i' r'. (i, r) \<in> regions \<longrightarrow> (i', r') \<in> regions \<longrightarrow> r \<bowtie> r' = (i \<noteq> i' \<and> (i, i') \<notin> {}))
\<close>
...
definition Q_ret_address_16 :: state_pred where
  \<open>Q_ret_address_16 \<sigma> \<equiv> RIP \<sigma> = ret_address \<and> RSI \<sigma> = (sextend (ucast ((\<langle>31,0\<rangle>RDX\<^sub>0::32 word))) 32 64::64 word) \<and> RSP \<sigma> = ((RSP\<^sub>0::64 word) + 0x8) \<and> (\<sigma> \<turnstile> *[RSP\<^sub>0,8]::64 word) = ret_address \<and> (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word) = (\<sigma> \<turnstile> *[((RDI\<^sub>0::64 word) + 0x850),8]::64 word)\<close>
declare Q_ret_address_16_def[Qs]

schematic_goal bdrv_get_backing_filename_0_1_0x559b_0x559b_16[blocks]:
  assumes \<open>(P_0x559b_16 && P_0x559b_16_regions) \<sigma>\<close>
  shows \<open>exec_block (Suc 0) 0x559b 16 \<sigma> \<triangleq> ?\<sigma> \<and> Q_ret_address_16 ?\<sigma> \<and> block_usage P_0x559b_16_regions_set \<sigma> ?\<sigma>\<close>
  using assms
  by (steps pre: P_0x559b_16_def P_0x559b_16_regions_def post: Q_ret_address_16_def regionset: P_0x559b_16_regions_set_def)

definition bdrv_get_backing_filename_acode :: ACode where
  \<open>bdrv_get_backing_filename_acode =
    Block 3 0x19aab 0;
    IF ZF THEN
      Block 4 0x19aca (Suc 0)
    ELSE
      Block 4 0x19aba 2
    FI;
    Block (Suc 0) 0x5560 3;
    IF ZF THEN
      Block (Suc 0) 0x5563 4
    ELSE
      Block 4 0x556c 5;
      IF ZF THEN
        Block (Suc 0) 0x556e 6
      ELSE
        Block 3 0x5575 7;
        IF CF THEN
          Block (Suc 0) 0x5578 8;
          WHILE P_0x5585_9 DO
            Block 5 0x5594 9;
            IF !ZF THEN
              Block 2 0x5580 10;
              ...
              ELSE
                Block (Suc 0) 0x5583 12
              FI
            ELSE
              Block (Suc 0) 0x5596 13
            FI
          OD
        ELSE
          Block 2 0x557a 14
        FI
      FI;
      Block (Suc 0) 0x5598 15
    FI;
    Block (Suc 0) 0x559b 16
  \<close>

schematic_goal "bdrv_get_backing_filename":
  assumes
    \<open>\<forall>\<sigma>. RIP \<sigma> = 0x19aae \<longrightarrow> P_0x19aae_true_1_regions \<sigma>\<close>
    and \<open>\<forall>\<sigma>. RIP \<sigma> = 0x19aae \<longrightarrow> P_0x19aae_false_2_regions \<sigma>\<close>
    and \<open>\<forall>\<sigma>. RIP \<sigma> = 0x5560 \<longrightarrow> P_0x5560_3_regions \<sigma>\<close>
    and \<open>\<forall>\<sigma>. RIP \<sigma> = 0x5563 \<longrightarrow> P_0x5563_true_4_regions \<sigma>\<close>
    and \<open>\<forall>\<sigma>. RIP \<sigma> = 0x5563 \<longrightarrow> P_0x5563_false_5_regions \<sigma>\<close>
    and \<open>\<forall>\<sigma>. RIP \<sigma> = 0x556e \<longrightarrow> P_0x556e_true_6_regions \<sigma>\<close>
    and \<open>\<forall>\<sigma>. RIP \<sigma> = 0x556e \<longrightarrow> P_0x556e_false_7_regions \<sigma>\<close>
    and \<open>\<forall>\<sigma>. RIP \<sigma> = 0x5578 \<longrightarrow> P_0x5578_true_8_regions \<sigma>\<close>
    and \<open>\<forall>\<sigma>. RIP \<sigma> = 0x5585 \<longrightarrow> P_0x5585_9_regions \<sigma>\<close>
    and \<open>\<forall>\<sigma>. RIP \<sigma> = 0x5596 \<longrightarrow> P_0x5596_true_10_regions \<sigma>\<close>
    and \<open>\<forall>\<sigma>. RIP \<sigma> = 0x5583 \<longrightarrow> P_0x5583_true_11_regions \<sigma>\<close>
    and \<open>\<forall>\<sigma>. RIP \<sigma> = 0x5583 \<longrightarrow> P_0x5583_false_12_regions \<sigma>\<close>
    and \<open>\<forall>\<sigma>. RIP \<sigma> = 0x5596 \<longrightarrow> P_0x5596_false_13_regions \<sigma>\<close>
    and \<open>\<forall>\<sigma>. RIP \<sigma> = 0x5578 \<longrightarrow> P_0x5578_false_14_regions \<sigma>\<close>
    and \<open>\<forall>\<sigma>. RIP \<sigma> = 0x5598 \<longrightarrow> P_0x5598_15_regions \<sigma>\<close>
    and \<open>\<forall>\<sigma>. RIP \<sigma> = 0x559b \<longrightarrow> P_0x559b_16_regions \<sigma>\<close>
  shows \<open>{{?P}} bdrv_get_backing_filename_acode {{?Q;?M}}\<close>
  apply (vcg acode: bdrv_get_backing_filename_acode_def assms: assms)
                   apply (vcg_while \<open>P_0x5585_9 || P_0x5598_15\<close> assms: assms)
                    apply (vcg_step assms: assms)+

#Derived work from Ramblr. Modified to support formal verification annotations
...
class SymbolManager(object):
    ...
    def new_label(self, addr, name=None, is_function=None, force=False):
        ...
        if addr in reverse_plt:
        ...
        elif symbol and symbol.owner is self.project.loader.main_object:
            ...
            elif symbol.type == cle.Symbol.TYPE_OTHER:
                label = OtherLabel(self.binary, symbol_name, addr)
...
class Instruction(object):
    ...
    def __init__(self, binary, addr, size, insn_bytes, capstone_instr):
        ...
        self.opcode = capstone_instr.opcode
...
#       comments = ( "\t## Size:%s" % (self.size) ) + comments
    ...
    def assembly(self, comments=False, symbolized=True):
        ...
        not_symbolized = "\t%s\t%s # Size:%s, Opcode: 0x%02x,0x%02x,0x%02x,0x%02x" % (self.mnemonic, self.op_str, self.size, self.opcode[0], self.opcode[1], self.opcode[2], self.opcode[3])
        if not symbolized:
            asm = not_symbolized
            ...
            asm = "\t%s%s # Size:%s, Opcode: 0x%02x,0x%02x,0x%02x,0x%02x" % (mnemonic, "\t" + ", ".join(all_operands), self.size, self.opcode[0], self.opcode[1], self.opcode[2], self.opcode[3])
...
class Procedure(object):
    ...
    def assembly(self, comments=False, symbolized=True):
        ...
        header = "\t.section\t{section}\n\t.align\t{alignment}".format(section=self.section,
        ...
        if self.asm_code:
            ...
            l=len(self.blocks)
            for c,b in enumerate(sorted(self.blocks, key=lambda x:x.addr)):  # type: BasicBlock
                s = b.assembly(comments=comments, symbolized=symbolized)
                if (c+1) == l: #if last block
                    s+="\n"
...
class Data(object):
    ...
    def is_unnecessary(self):
        ...
        if self.section_name == ".plt":
            return True
        if self.section_name == ".plt.got":
            return True
        if self.section_name == ".text": #There should be no data sections of type text.
            return True
        #if self.section_name == ".bss":
        #    return True
        if any(lbl.name in glibc_data_blacklist for (_, lbl) in self.labels):
            return True
...
#            s += "## data (%s)\n" % self.name
        ...
        #if self.sort == 'string':
        if self.sort != self.sort: # Process strings as bytes instead
...
class Reassembler(Analysis):
    ...
    """
    def __init__(self, syntax="intel", reassemblable=True, remove_cgc_attachments=True, log_relocations=True, ):
        ...
        self.reassemblable = reassemblable
        self.procedures = [ ]
        self.data = [ ]
    ...
    def assembly(self, comments=False, symbolized=True):
        ...
        if self.reassemblable:
            self.remove_unnecessary_stuff()
    ...
    def remove_unnecessary_stuff(self):
        ...
        self.procedures = [p for p in self.procedures if p.name not in glibc_functions_blacklist and not p.is_plt and not p.subfunc]

        self.data = [d for d in self.data if not d.is_unnecessary]
...
#                        d.content[i] = 0
    ...
    def _initialize(self):
        ...
        section_names_to_ignore = {'.init', '.fini', '.fini_array', '.jcr', '.dynamic', '.got', '.got.plt',
                                   ...
                                   '.gnu.version', '.gnu.version_r', 'plt.got'
                                   }

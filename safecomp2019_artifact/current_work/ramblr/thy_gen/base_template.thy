...
*)
theory {infile}
  imports "{isabelle_path}/Presimplified_Semantics_Manual2"
          "HOL-Library.Rewrite"
          reassembly_setup.Abstract_System
          reassembly_setup.Det_Step
begin

text \<open>Load the \texttt{{{infile}.s}} file.\<close>
x86_64_parser "{rel_outpath}/{infile}.s"
...
text \<open>
  A locale building a context for {infile}.s. This locale can be auto-generated.
\<close>
locale {infile}_context = {infile} + presimplified_semantics
begin
...
fun {infile}_flag_annotation :: flag_annotation where
  \<open>{infile}_flag_annotation loc = ({flag_annotations})\<close>
...
definition step :: \<open>(unit, state) se\<close> where
  \<open>step \<sigma> \<equiv>
    ...
    let' (_, i, s) = text_sections \<alpha>!index;
         \<sigma> = exec_instr \<alpha> semantics {infile}_flag_annotation i s \<sigma>
    in
      Some ((), \<sigma>)\<close>

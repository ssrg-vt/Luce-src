...
*)
theory task_list_push_back_mem
  imports tasks
begin

text \<open>Up to two locales per function in the binary.\<close>
locale task_list_push_back_function = tasks_context +
  fixes rsp\<^sub>0 rbp\<^sub>0 a task_list_push_back_ret :: \<open>64 word\<close>
  ...
  assumes seps: \<open>seps blocks\<close>
    ...
    and ret_address: \<open>outside task_list_push_back_ret 139 317\<close> \<comment> \<open>Only works for non-recursive functions.\<close>
begin
...
definition pp_\<Theta> :: \<open>_ \<Rightarrow> _ \<Rightarrow> _ \<Rightarrow> floyd_invar\<close> where
  \<open>pp_\<Theta> list task last' \<equiv> [
    ...
    boffset+139 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0
      \<and> regs \<sigma> rbp = rbp\<^sub>0
      \<and> regs \<sigma> rdi = list
      \<and> regs \<sigma> rsi = task
      \<and> \<sigma> \<turnstile> *[list + 8,8] = last'
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+task_list_push_back_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+314 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-8
      ...
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+task_list_push_back_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+315 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-8
      \<and> regs \<sigma> rbp = rsp\<^sub>0-8
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-8,8] = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+task_list_push_back_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    \<comment> \<open>postcondition\<close>
    boffset+task_list_push_back_ret \<mapsto> \<lambda>\<sigma>. \<sigma> \<turnstile> *[a,1] = v\<^sub>0
...
lemma rewrite_task_list_push_back_mem:
  assumes
    \<open>master blocks (list, 8) 5\<close>
    \<open>master blocks (list + 8, 8) 6\<close>
    \<open>master blocks (task + 0x58, 8) 7\<close>
    \<open>master blocks (task + 0x60, 8) 8\<close>
    \<open>master blocks (last' + 0x58, 8) 9\<close>
  shows \<open>is_std_invar task_list_push_back_ret (floyd.invar task_list_push_back_ret (pp_\<Theta> list task last'))\<close>
proof -
  ...
  show ?thesis
    ...
    apply (rewrite at \<open>floyd_vcs task_list_push_back_ret \<hole> _\<close> pp_\<Theta>_def)
    apply (intro floyd_vcsI)

    text \<open>Subgoal for rip = boffset+139\<close>
    ...
    text \<open>Subgoal for rip = boffset+314\<close>
    subgoal premises prems for \<sigma>
      text \<open>Insert relevant knowledge\<close>
      apply (insert prems seps ret_address)
      text \<open>Apply VCG/symb.\ execution\<close>
    ...
    text \<open>Subgoal for rip = boffset+315\<close>
    subgoal premises prems for \<sigma>
      text \<open>Insert relevant knowledge\<close>
      apply (insert prems seps ret_address)
      text \<open>Apply VCG/symb.\ execution\<close>

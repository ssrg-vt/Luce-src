...
text \<open>Load the \texttt{tasks_opt.s} file.\<close>
x86_64_parser "../examples/hermitcore/tasks/tasks_opt.s"
...
text \<open>
  A locale building a context for tasks_opt.s. This locale can be auto-generated.
\<close>
locale tasks_opt_context = tasks_opt + presimplified_semantics
begin
...
text \<open>Generated automatically for cmp and test; some of the optimized ones may be inaccurate.\<close>
fun tasks_opt_flag_annotation :: flag_annotation where
  \<open>tasks_opt_flag_annotation loc = (if loc \<in> {
    boffset + 11,
    boffset + 24,
    boffset + 33,
    boffset + 39,
...
definition step :: \<open>(unit, state) se\<close> where
  \<open>step \<sigma> \<equiv>
    ...
    let' (_, i, s) = text_sections \<alpha>!index;
         \<sigma> = exec_instr \<alpha> semantics tasks_opt_flag_annotation i s \<sigma>
    in
      Some ((), \<sigma>)\<close>

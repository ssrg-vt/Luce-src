...
text \<open>Load the \texttt{virtio_ring.s} file.\<close>
x86_64_parser "../examples/hermitcore/virtio_ring/virtio_ring.s"
...
text \<open>
  A locale building a context for virtio_ring.s. This locale can be auto-generated.
\<close>
locale virtio_ring_context = virtio_ring + presimplified_semantics
begin
...
fun virtio_ring_flag_annotation :: flag_annotation where
  \<open>virtio_ring_flag_annotation loc = (if loc \<in> {
    boffset + 233
...
definition step :: \<open>(unit, state) se\<close> where
  \<open>step \<sigma> \<equiv>
    ...
    let' (_, i, s) = text_sections \<alpha>!index;
         \<sigma> = exec_instr \<alpha> semantics virtio_ring_flag_annotation i s \<sigma>
    in
      Some ((), \<sigma>)\<close>

...
text \<open>Load the \texttt{virtio_ring_opt.s} file.\<close>
x86_64_parser "../examples/hermitcore/virtio_ring/virtio_ring_opt.s"
...
text \<open>
  A locale building a context for virtio_ring_opt.s. This locale can be auto-generated.
\<close>
locale virtio_ring_opt_context = virtio_ring_opt + presimplified_semantics
begin
...
fun virtio_ring_opt_flag_annotation :: flag_annotation where
  \<open>virtio_ring_opt_flag_annotation loc = (if loc \<in> {
    boffset + 72
  } then {flag_cf} else {})\<close>
...
definition step :: \<open>(unit, state) se\<close> where
  \<open>step \<sigma> \<equiv>
    ...
    let' (_, i, s) = text_sections \<alpha>!index;
         \<sigma> = exec_instr \<alpha> semantics virtio_ring_opt_flag_annotation i s \<sigma>
    in
      Some ((), \<sigma>)\<close>

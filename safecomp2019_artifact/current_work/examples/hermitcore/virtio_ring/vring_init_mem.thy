...
*)
theory vring_init_mem
  imports virtio_ring
begin

text \<open>Up to two locales per function in the binary.\<close>
locale vring_init_function = virtio_ring_context +
  fixes rsp\<^sub>0 rbp\<^sub>0 vr a vring_init_ret :: \<open>64 word\<close>
  ...
  assumes seps: \<open>seps blocks\<close>
    and masters:
      ...
      \<open>master blocks (vr, 4) 7\<close>
      \<open>master blocks (vr+8, 8) 8\<close>
      \<open>master blocks (vr+16, 8) 9\<close>
      \<open>master blocks (vr+24, 8) 10\<close>
    and ret_address: \<open>outside vring_init_ret 0 124\<close> \<comment> \<open>Only works for non-recursive functions.\<close>
begin
...
definition pp_\<Theta> :: floyd_invar where
  \<open>pp_\<Theta> \<equiv> [
    ...
    boffset \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0
      ...
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+vring_init_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    \<comment> \<open>postcondition\<close>
    boffset+vring_init_ret \<mapsto> \<lambda>\<sigma>. \<sigma> \<turnstile> *[a,1] = v\<^sub>0
...
lemma rewrite_vring_init_mem:
  \<open>is_std_invar vring_init_ret (floyd.invar vring_init_ret pp_\<Theta>)\<close>
  text \<open>Boilerplate code to start the VCG\<close>
  apply (rule floyd_invarI)
  apply (rewrite at \<open>floyd_vcs vring_init_ret \<hole> _\<close> pp_\<Theta>_def)

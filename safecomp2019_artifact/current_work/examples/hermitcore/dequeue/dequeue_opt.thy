...
text \<open>Load the \texttt{dequeue_opt.s} file.\<close>
...
text \<open>
  A locale building a context for dequeue_opt.s. This locale can be auto-generated.
\<close>
locale dequeue_opt_context = dequeue_opt + presimplified_semantics
begin
...
fun dequeue_opt_flag_annotation :: flag_annotation where
  \<open>dequeue_opt_flag_annotation loc = (if loc \<in> {
    boffset + 177,
    boffset + 241,
    boffset + 293, \<comment> \<open>These probably aren't right due to the rearranging.\<close>
    boffset + 385,
    boffset + 390,
    boffset + 420
...
definition step :: \<open>(unit, state) se\<close> where
  \<open>step \<sigma> \<equiv>
    ...
    let' (_, i, s) = text_sections \<alpha>!index;
         \<sigma> = exec_instr \<alpha> semantics dequeue_opt_flag_annotation i s \<sigma>
    in
      Some ((), \<sigma>)\<close>

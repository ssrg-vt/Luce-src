...
locale dequeue_init_function = dequeue_context +
  fixes rsp\<^sub>0 rbp\<^sub>0 a dequeue_init_ret :: \<open>64 word\<close>
  ...
  assumes seps: \<open>seps blocks\<close>
    ...
    and ret_address: \<open>outside dequeue_init_ret 0 129\<close> \<comment> \<open>Only works for non-recursive functions.\<close>
begin
...
definition pp_\<Theta> :: \<open>_ \<Rightarrow> floyd_invar\<close> where
  \<open>pp_\<Theta> dequeue \<equiv> [
    ...
    boffset \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0
      ...
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+dequeue_init_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+117 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-40
      \<and> regs \<sigma> rbp = rsp\<^sub>0-8
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-8,8] = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+dequeue_init_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+122 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-40
      \<and> regs \<sigma> rbp = rsp\<^sub>0-8
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-8,8] = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+dequeue_init_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+127 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-40
      \<and> regs \<sigma> rbp = rsp\<^sub>0-8
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-8,8] = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+dequeue_init_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    \<comment> \<open>postcondition\<close>
    boffset+dequeue_init_ret \<mapsto> \<lambda>\<sigma>. \<sigma> \<turnstile> *[a,1] = v\<^sub>0
...
locale dequeue_init_function_calls = dequeue_init_function +
  ...
  assumes spinlock_init_def: \<open>exec_instr assembly semantics tasks_flag_annotation (Unary (IS_8088 Call) (Immediate (ImmLabel ''spinlock_init''))) 5 = spinlock_init\<close>
    ...
    and spinlock_init117: \<open>the (pp_\<Theta> dequeue (boffset+117)) \<sigma> \<Longrightarrow> the (pp_\<Theta> dequeue (boffset+122)) (spinlock_init \<sigma>)\<close>
begin

lemma rewrite_dequeue_init_mem:
  assumes
    \<open>master blocks (dequeue, 8) 7\<close>
    \<open>master blocks (dequeue + 8, 8) 8\<close>
    \<open>master blocks (dequeue + 40, 8) 9\<close>
    \<open>master blocks (dequeue + 48, 8) 10\<close>
    \<open>master blocks (dequeue + 56, 8) 11\<close>
  shows \<open>is_std_invar dequeue_init_ret (floyd.invar dequeue_init_ret (pp_\<Theta> dequeue))\<close>
proof -
  ...
  show ?thesis
    ...
    apply (rewrite at \<open>floyd_vcs dequeue_init_ret \<hole> _\<close> pp_\<Theta>_def)
    ...
    text \<open>Subgoal for rip = boffset+117\<close>
    subgoal premises prems for \<sigma>
      text \<open>Insert relevant knowledge\<close>
      apply (insert prems seps ret_address)
      ...
      apply (rule floyd_invarI'[of _ _ \<open>the (pp_\<Theta> dequeue (boffset+122))\<close>])
       apply (simp add: spinlock_init_incr)
      using spinlock_init117
      apply simp
      done

    text \<open>Subgoal for rip = boffset+122\<close>
    subgoal premises prems for \<sigma>
      text \<open>Insert relevant knowledge\<close>
      apply (insert prems seps ret_address)
      text \<open>Apply VCG/symb.\ execution\<close>
    ...
    text \<open>Subgoal for rip = boffset+127\<close>
    subgoal premises prems for \<sigma>
      text \<open>Insert relevant knowledge\<close>
      apply (insert prems seps ret_address)
      text \<open>Apply VCG/symb.\ execution\<close>

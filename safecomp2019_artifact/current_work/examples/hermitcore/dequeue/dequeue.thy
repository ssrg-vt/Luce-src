...
text \<open>Load the \texttt{dequeue.s} file.\<close>
x86_64_parser "../examples/hermitcore/dequeue/dequeue.s"
...
text \<open>
  A locale building a context for dequeue.s. This locale can be auto-generated.
\<close>
locale dequeue_context = dequeue + presimplified_semantics
begin
...
fun dequeue_flag_annotation :: flag_annotation where
  \<open>dequeue_flag_annotation loc = (if loc \<in> {
    boffset + 24,
    boffset + 35,
    boffset + 145,
    boffset + 156,
...
definition step :: \<open>(unit, state) se\<close> where
  \<open>step \<sigma> \<equiv>
    ...
    let' (_, i, s) = text_sections \<alpha>!index;
         \<sigma> = exec_instr \<alpha> semantics dequeue_flag_annotation i s \<sigma>
    in
      Some ((), \<sigma>)\<close>
...
find_consts name: is_result

thm floyd.invar_def
thm floyd.isys.yields'_def
thm floyd.isys.is_result_def
thm floyd.isys.step_rel_def
thm floyd.isys.terminated.simps
thm floyd.has_invar_def
thm linvar_def

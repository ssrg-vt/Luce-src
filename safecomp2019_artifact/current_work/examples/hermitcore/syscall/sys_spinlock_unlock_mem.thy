...
*)
theory sys_spinlock_unlock_mem
  imports syscall
begin

text \<open>Up to two locales per function in the binary.\<close>
locale sys_spinlock_unlock_function = syscall_context +
  fixes rsp\<^sub>0 rbp\<^sub>0 a sys_spinlock_unlock_ret :: \<open>64 word\<close>
  ...
  assumes seps: \<open>seps blocks\<close>
    ...
    and ret_address: \<open>outside sys_spinlock_unlock_ret 2433 2482\<close> \<comment> \<open>Only works for non-recursive functions.\<close>
begin
...
definition pp_\<Theta> :: floyd_invar where
  \<open>pp_\<Theta> \<equiv> [
    ...
    boffset+2433 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0
      \<and> regs \<sigma> rbp = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+sys_spinlock_unlock_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+2475 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-24
      \<and> regs \<sigma> rbp = rsp\<^sub>0-8
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-8,8] = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+sys_spinlock_unlock_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+2480 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-24
      \<and> regs \<sigma> rbp = rsp\<^sub>0-8
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-8,8] = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+sys_spinlock_unlock_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    \<comment> \<open>postcondition\<close>
    boffset+sys_spinlock_unlock_ret \<mapsto> \<lambda>\<sigma>. \<sigma> \<turnstile> *[a,1] = v\<^sub>0
...
locale sys_spinlock_unlock_function_calls = sys_spinlock_unlock_function +
  fixes spinlock_unlock :: \<open>state \<Rightarrow> state\<close>
  assumes spinlock_unlock_def: \<open>exec_instr assembly semantics tasks_flag_annotation (Unary (IS_8088 Call) (Immediate (ImmLabel ''spinlock_unlock''))) 5 = spinlock_unlock\<close>
    ...
    and spinlock_unlock2475: \<open>the (pp_\<Theta> (boffset+2475)) \<sigma> \<Longrightarrow> the (pp_\<Theta> (boffset+2480)) (spinlock_unlock \<sigma>)\<close>
begin

lemma rewrite_sys_spinlock_unlock_mem:
  \<open>is_std_invar sys_spinlock_unlock_ret (floyd.invar sys_spinlock_unlock_ret pp_\<Theta>)\<close>
  text \<open>Boilerplate code to start the VCG\<close>
  apply (rule floyd_invarI)
  apply (rewrite at \<open>floyd_vcs sys_spinlock_unlock_ret \<hole> _\<close> pp_\<Theta>_def)
  apply (intro floyd_vcsI)

  text \<open>Subgoal for rip = boffset+2433\<close>
  ...
  text \<open>Subgoal for rip = boffset+2475\<close>
  subgoal premises prems for \<sigma>
    text \<open>Insert relevant knowledge\<close>
    apply (insert prems seps ret_address)
    ...
    apply (rule floyd_invarI'[of _ _ \<open>the (pp_\<Theta> (boffset+2480))\<close>])
     apply (simp add: spinlock_unlock_incr)
    using spinlock_unlock2475
    apply simp
    done

  text \<open>Subgoal for rip = boffset+2480\<close>
  subgoal premises prems for \<sigma>
    text \<open>Insert relevant knowledge\<close>
    apply (insert prems seps ret_address)
    text \<open>Apply VCG/symb.\ execution\<close>

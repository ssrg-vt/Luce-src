...
*)
theory sys_sem_wait_mem
  imports syscall
begin

text \<open>Up to two locales per function in the binary.\<close>
locale sys_sem_wait_function = syscall_context +
  fixes rsp\<^sub>0 rbp\<^sub>0 a sys_sem_wait_ret :: \<open>64 word\<close>
  ...
  assumes seps: \<open>seps blocks\<close>
    ...
    and ret_address: \<open>outside sys_sem_wait_ret 2701 2755\<close> \<comment> \<open>Only works for non-recursive functions.\<close>
begin
...
definition pp_\<Theta> :: floyd_invar where
  \<open>pp_\<Theta> \<equiv> [
    ...
    boffset+2701 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0
      \<and> regs \<sigma> rbp = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+sys_sem_wait_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+2748 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-24
      \<and> regs \<sigma> rbp = rsp\<^sub>0-8
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-8,8] = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+sys_sem_wait_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+2753 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-24
      \<and> regs \<sigma> rbp = rsp\<^sub>0-8
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-8,8] = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+sys_sem_wait_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    \<comment> \<open>postcondition\<close>
    boffset+sys_sem_wait_ret \<mapsto> \<lambda>\<sigma>. \<sigma> \<turnstile> *[a,1] = v\<^sub>0
...
locale sys_sem_wait_function_calls = sys_sem_wait_function +
  ...
  assumes sem_wait_def: \<open>exec_instr assembly semantics tasks_flag_annotation (Unary (IS_8088 Call) (Immediate (ImmLabel ''sem_wait''))) 5 = sem_wait\<close>
    ...
    and sem_wait2748: \<open>the (pp_\<Theta> (boffset+2748)) \<sigma> \<Longrightarrow> the (pp_\<Theta> (boffset+2753)) (sem_wait \<sigma>)\<close>
begin

lemma rewrite_sys_sem_wait_mem:
  \<open>is_std_invar sys_sem_wait_ret (floyd.invar sys_sem_wait_ret pp_\<Theta>)\<close>
  text \<open>Boilerplate code to start the VCG\<close>
  apply (rule floyd_invarI)
  apply (rewrite at \<open>floyd_vcs sys_sem_wait_ret \<hole> _\<close> pp_\<Theta>_def)
  apply (intro floyd_vcsI)

  text \<open>Subgoal for rip = boffset+2701\<close>
  ...
  text \<open>Subgoal for rip = boffset+2748\<close>
  subgoal premises prems for \<sigma>
    text \<open>Insert relevant knowledge\<close>
    apply (insert prems seps ret_address)
    ...
    apply (rule floyd_invarI'[of _ _ \<open>the (pp_\<Theta> (boffset+2753))\<close>])
     apply (simp add: sem_wait_incr)
    using sem_wait2748
    apply simp
    done

  text \<open>Subgoal for rip = boffset+2753\<close>
  subgoal premises prems for \<sigma>
    text \<open>Insert relevant knowledge\<close>
    apply (insert prems seps ret_address)
    text \<open>Apply VCG/symb.\ execution\<close>

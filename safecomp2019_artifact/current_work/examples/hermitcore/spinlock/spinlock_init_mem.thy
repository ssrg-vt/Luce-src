...
*)
theory spinlock_init_mem
  imports spinlock
begin

text \<open>Up to two locales per function in the binary.\<close>
locale spinlock_init_function = spinlock_context +
  fixes rsp\<^sub>0 rbp\<^sub>0 a spinlock_init_ret :: \<open>64 word\<close>
  ...
  assumes seps: \<open>seps blocks\<close>
    ...
    and ret_address: \<open>outside spinlock_init_ret 146 248\<close> \<comment> \<open>Only works for non-recursive functions.\<close>
begin
...
definition pp_\<Theta> :: \<open>_ \<Rightarrow> floyd_invar\<close> where
  \<open>pp_\<Theta> s \<equiv> [
    ...
    boffset+146 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0
      ...
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+spinlock_init_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+193 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-16
      \<and> regs \<sigma> rbp = rsp\<^sub>0-8
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-16,8] = s
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-8,8] = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+spinlock_init_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+198 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-16
      \<and> regs \<sigma> rbp = rsp\<^sub>0-8
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-16,8] = s
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-8,8] = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+spinlock_init_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+214 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-16
      \<and> regs \<sigma> rbp = rsp\<^sub>0-8
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-16,8] = s
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-8,8] = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+spinlock_init_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+219 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-16
      \<and> regs \<sigma> rbp = rsp\<^sub>0-8
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-16,8] = s
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-8,8] = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+spinlock_init_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+246 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-16
      \<and> regs \<sigma> rbp = rsp\<^sub>0-8
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-8,8] = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+spinlock_init_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    \<comment> \<open>postcondition\<close>
    boffset+spinlock_init_ret \<mapsto> \<lambda>\<sigma>. \<sigma> \<turnstile> *[a,1] = v\<^sub>0
...
locale spinlock_init_function_calls = spinlock_init_function +
  ...
  assumes atomic_int64_set_def: \<open>exec_instr assembly semantics tasks_flag_annotation (Unary (IS_8088 Call) (Immediate (ImmLabel ''atomic_int64_set''))) 5 = atomic_int64_set\<close>
    ...
    and atomic_int64_set193: \<open>the (pp_\<Theta> s (boffset+193)) \<sigma> \<Longrightarrow> the (pp_\<Theta> s (boffset+198)) (atomic_int64_set \<sigma>)\<close>
    and atomic_int64_set214: \<open>the (pp_\<Theta> s (boffset+214)) \<sigma> \<Longrightarrow> the (pp_\<Theta> s (boffset+219)) (atomic_int64_set \<sigma>)\<close>
begin

lemma rewrite_spinlock_init_mem:
  assumes
    \<open>master blocks (s + 16, 4) 4\<close>
    \<open>master blocks (s + 20, 4) 5\<close>
  shows \<open>is_std_invar spinlock_init_ret (floyd.invar spinlock_init_ret (pp_\<Theta> s))\<close>
proof -
  ...
  show ?thesis
    ...
    apply (rewrite at \<open>floyd_vcs spinlock_init_ret \<hole> _\<close> pp_\<Theta>_def)
    ...
    text \<open>Subgoal for rip = boffset+193\<close>
    subgoal premises prems for \<sigma>
      text \<open>Insert relevant knowledge\<close>
      apply (insert prems seps ret_address)
      ...
      apply (rule floyd_invarI'[of _ _ \<open>the (pp_\<Theta> s (boffset+198))\<close>])
       apply (simp add: atomic_int64_set_incr)
      using atomic_int64_set193
      apply simp
      done

    text \<open>Subgoal for rip = boffset+198\<close>
    subgoal premises prems for \<sigma>
      text \<open>Insert relevant knowledge\<close>
      apply (insert prems seps ret_address)
      text \<open>Apply VCG/symb.\ execution\<close>
    ...
    text \<open>Subgoal for rip = boffset+214\<close>
    subgoal premises prems for \<sigma>
      text \<open>Insert relevant knowledge\<close>
      apply (insert prems seps ret_address)
      text \<open>Discharge function call\<close>
      ...
      apply (rewrite_one_let' add: assembly_def)
      apply (rewrite_one_let' add: atomic_int64_set_def)
      apply (rule floyd_invarI'[of _ _ \<open>the (pp_\<Theta> s (boffset+219))\<close>])
       apply (simp add: atomic_int64_set_incr)
      using atomic_int64_set214
      apply simp
      done

    text \<open>Subgoal for rip = boffset+219\<close>
    subgoal premises prems for \<sigma>
      text \<open>Insert relevant knowledge\<close>
      apply (insert prems seps ret_address)
      text \<open>Apply VCG/symb.\ execution\<close>

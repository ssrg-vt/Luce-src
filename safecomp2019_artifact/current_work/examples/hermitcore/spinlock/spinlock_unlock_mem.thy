...
*)
theory spinlock_unlock_mem
  imports spinlock
begin

text \<open>Up to two locales per function in the binary.\<close>
locale spinlock_unlock_function = spinlock_context +
  fixes rsp\<^sub>0 rbp\<^sub>0 a spinlock_unlock_ret :: \<open>64 word\<close>
  ...
  assumes seps: \<open>seps blocks\<close>
    ...
    and ret_address: \<open>outside spinlock_unlock_ret 468 565\<close> \<comment> \<open>Only works for non-recursive functions.\<close>
begin
...
definition pp_\<Theta> :: \<open>_ \<Rightarrow> floyd_invar\<close> where
  \<open>pp_\<Theta> s \<equiv> [
    ...
    boffset+468 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0
      \<and> regs \<sigma> rbp = rbp\<^sub>0
      \<and> regs \<sigma> rdi = s
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+spinlock_unlock_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+553 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-16
      \<and> regs \<sigma> rbp = rsp\<^sub>0-8
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-8,8] = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+spinlock_unlock_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+558 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-16
      \<and> regs \<sigma> rbp = rsp\<^sub>0-8
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-8,8] = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+spinlock_unlock_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+563 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-16
      \<and> regs \<sigma> rbp = rsp\<^sub>0-8
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-8,8] = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+spinlock_unlock_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    \<comment> \<open>postcondition\<close>
    boffset+spinlock_unlock_ret \<mapsto> \<lambda>\<sigma>. \<sigma> \<turnstile> *[a,1] = v\<^sub>0
...
locale spinlock_unlock_function_calls = spinlock_unlock_function +
  fixes atomic_int64_inc :: \<open>state \<Rightarrow> state\<close>
  assumes atomic_int64_inc_def: \<open>exec_instr assembly semantics tasks_flag_annotation (Unary (IS_8088 Call) (Immediate (ImmLabel ''atomic_int64_inc''))) 5 = atomic_int64_inc\<close>
    and atomic_int64_inc_incr: \<open>regs (atomic_int64_inc \<sigma>) rip = regs \<sigma> rip + 5\<close>
    and atomic_int64_inc553: \<open>the (pp_\<Theta> s (boffset+553)) \<sigma> \<Longrightarrow> the (pp_\<Theta> s (boffset+558)) (atomic_int64_inc \<sigma>)\<close>
begin

lemma rewrite_spinlock_unlock_mem:
  assumes
    \<open>master blocks (s + 16, 4) 4\<close>
    \<open>master blocks (s + 20, 4) 5\<close>
  shows \<open>is_std_invar spinlock_unlock_ret (floyd.invar spinlock_unlock_ret (pp_\<Theta> s))\<close>
proof -
  ...
  show ?thesis
    ...
    apply (rewrite at \<open>floyd_vcs spinlock_unlock_ret \<hole> _\<close> pp_\<Theta>_def)
    ...
    text \<open>Subgoal for rip = boffset+553\<close>
    subgoal premises prems for \<sigma>
      text \<open>Insert relevant knowledge\<close>
      apply (insert prems seps ret_address)
      ...
      apply (rule floyd_invarI'[of _ _ \<open>the (pp_\<Theta> s (boffset+558))\<close>])
       apply (simp add: atomic_int64_inc_incr)
      using atomic_int64_inc553
      apply simp
      done

    text \<open>Subgoal for rip = boffset+558\<close>
    subgoal premises prems for \<sigma>
      text \<open>Insert relevant knowledge\<close>
      apply (insert prems seps ret_address)
      text \<open>Apply VCG/symb.\ execution\<close>
    ...
    text \<open>Subgoal for rip = boffset+563\<close>
    subgoal premises prems for \<sigma>
      text \<open>Insert relevant knowledge\<close>
      apply (insert prems seps ret_address)
      text \<open>Apply VCG/symb.\ execution\<close>

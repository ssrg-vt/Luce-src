...
text \<open>Load the \texttt{spinlock.s} file.\<close>
x86_64_parser "../examples/hermitcore/spinlock/spinlock.s"
...
text \<open>
  A locale building a context for spinlock.s. This locale can be auto-generated.
\<close>
locale spinlock_context = spinlock + presimplified_semantics
begin
...
fun spinlock_flag_annotation :: flag_annotation where
  \<open>spinlock_flag_annotation loc = (if loc \<in> {
    boffset + 158,
    boffset + 169,
    boffset + 256,
    boffset + 267,
...
definition step :: \<open>(unit, state) se\<close> where
  \<open>step \<sigma> \<equiv>
    ...
    let' (_, i, s) = text_sections \<alpha>!index;
         \<sigma> = exec_instr \<alpha> semantics spinlock_flag_annotation i s \<sigma>
    in
      Some ((), \<sigma>)\<close>

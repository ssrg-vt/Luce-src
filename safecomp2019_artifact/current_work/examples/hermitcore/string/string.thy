...
text \<open>Load the \texttt{string.s} file.\<close>
x86_64_parser "../examples/hermitcore/string/string.s"
...
text \<open>
  A locale building a context for string.s. This locale can be auto-generated.
\<close>
locale string_context = string + presimplified_semantics
begin
...
fun string_flag_annotation :: flag_annotation where
  \<open>string_flag_annotation loc = (if loc \<in> {
    boffset + 111,
    boffset + 195,
    boffset + 502,
    boffset + 530
...
definition step :: \<open>(unit, state) se\<close> where
  \<open>step \<sigma> \<equiv>
    ...
    let' (_, i, s) = text_sections \<alpha>!index;
         \<sigma> = exec_instr \<alpha> semantics string_flag_annotation i s \<sigma>
    in
      Some ((), \<sigma>)\<close>

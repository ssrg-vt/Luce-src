...
locale memcpy_function = string_context +
  fixes rsp\<^sub>0 rbp\<^sub>0 count a memcpy_ret :: \<open>64 word\<close>
  ...
  assumes seps: \<open>seps blocks\<close>
    ...
    and ret_address: \<open>outside memcpy_ret 11 123\<close> \<comment> \<open>Only works for non-recursive functions.\<close>
begin
...
definition pp_\<Theta> :: \<open>_ \<Rightarrow> floyd_invar\<close> where
  \<open>pp_\<Theta> dest \<equiv> [
    ...
    boffset+11 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0
      ...
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+memcpy_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+59 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-8
      ...
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+memcpy_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+107 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-8
      \<and> regs \<sigma> rbp = rsp\<^sub>0-8
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-48,8] = count
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-32,8] = dest
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-8,8] = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+memcpy_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+121 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-8
      \<and> regs \<sigma> rbp = rsp\<^sub>0-8
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-8,8] = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+memcpy_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    \<comment> \<open>postcondition\<close>
    boffset+memcpy_ret \<mapsto> \<lambda>\<sigma>. \<sigma> \<turnstile> *[a,1] = v\<^sub>0
...
lemma rewrite_memcpy_mem:
  assumes \<open>\<forall>i<count. master blocks (dest+i, 1) 7\<close>
  shows \<open>is_std_invar memcpy_ret (floyd.invar memcpy_ret (pp_\<Theta> dest))\<close>
  text \<open>Boilerplate code to start the VCG\<close>
  apply (rule floyd_invarI)
  apply (rewrite at \<open>floyd_vcs memcpy_ret \<hole> _\<close> pp_\<Theta>_def)
  ...
  text \<open>Subgoal for rip = boffset+107\<close>
  subgoal premises prems for \<sigma>
    text \<open>Insert relevant knowledge\<close>
    apply (insert prems seps ret_address)
    text \<open>Apply VCG/symb.\ execution\<close>
  ...
  text \<open>Subgoal for rip = boffset+121\<close>
  subgoal premises prems for \<sigma>
    text \<open>Insert relevant knowledge\<close>
    apply (insert prems seps ret_address)
    text \<open>Apply VCG/symb.\ execution\<close>

...
text \<open>Load the \texttt{buddy.s} file.\<close>
...
text \<open>
  A locale building a context for buddy.s. This locale can be auto-generated.
\<close>
locale buddy_context = buddy + presimplified_semantics
begin
...
fun buddy_flag_annotation :: flag_annotation where
  \<open>buddy_flag_annotation loc = (if loc \<in> {
    ...
    boffset + 115,
    boffset + 128
  } then {flag_zf, flag_sf, flag_of} else {})\<close>
...
definition step :: \<open>(unit, state) se\<close> where
  \<open>step \<sigma> \<equiv>
    ...
    let' (_, i, s) = text_sections \<alpha>!index;
         \<sigma> = exec_instr \<alpha> semantics buddy_flag_annotation i s \<sigma>
    in
      Some ((), \<sigma>)\<close>

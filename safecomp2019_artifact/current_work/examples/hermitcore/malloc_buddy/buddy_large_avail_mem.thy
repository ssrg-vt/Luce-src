...
*)
theory buddy_large_avail_mem
  imports buddy
begin

text \<open>Up to two locales per function in the binary.\<close>
locale buddy_large_avail_function = buddy_context +
  fixes rsp\<^sub>0 rbp\<^sub>0 a buddy_large_avail_ret :: \<open>64 word\<close>
  ...
  assumes seps: \<open>seps blocks\<close>
    and masters:
      ...
      \<open>master blocks (rsp\<^sub>0-12, Suc 0) 3\<close>
    and ret_address: \<open>outside buddy_large_avail_ret 0 72\<close> \<comment> \<open>Only works for non-recursive functions.\<close>
begin
...
definition pp_\<Theta> :: floyd_invar where
  \<open>pp_\<Theta> \<equiv> [
    ...
    boffset \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0
      ...
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+buddy_large_avail_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+21 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-8
      \<and> regs \<sigma> rbp = rsp\<^sub>0-8
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-8,8] = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+buddy_large_avail_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    boffset+60 \<mapsto> \<lambda>\<sigma>. regs \<sigma> rsp = rsp\<^sub>0-8
      \<and> regs \<sigma> rbp = rsp\<^sub>0-8
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0-8,8] = rbp\<^sub>0
      \<and> \<sigma> \<turnstile> *[rsp\<^sub>0,8] = boffset+buddy_large_avail_ret
      \<and> \<sigma> \<turnstile> *[a,1] = v\<^sub>0,
    \<comment> \<open>postcondition\<close>
    boffset+buddy_large_avail_ret \<mapsto> \<lambda>\<sigma>. \<sigma> \<turnstile> *[a,1] = v\<^sub>0
...
lemma rewrite_buddy_large_avail_mem:
  \<open>is_std_invar buddy_large_avail_ret (floyd.invar buddy_large_avail_ret pp_\<Theta>)\<close>
  text \<open>Boilerplate code to start the VCG\<close>
  apply (rule floyd_invarI)
  apply (rewrite at \<open>floyd_vcs buddy_large_avail_ret \<hole> _\<close> pp_\<Theta>_def)
  ...
  text \<open>Subgoal for rip = boffset+21\<close>
  subgoal premises prems for \<sigma>
  proof -
    ...
    show ?thesis
      ...
      apply ((restart_symbolic_execution add: x)?, (symbolic_execution add: x masters: masters)+, (finish_symbolic_execution add: x masters: masters)?)+
       apply (restart_symbolic_execution add: x)
       apply auto[]
      apply ((restart_symbolic_execution add: x)?, (symbolic_execution add: x masters: masters)+, (finish_symbolic_execution add: x masters: masters)?)+
      apply (restart_symbolic_execution add: x)
      apply auto[]
  ...
  text \<open>Subgoal for rip = boffset+60\<close>
  subgoal premises prems for \<sigma>
    text \<open>Insert relevant knowledge\<close>
    apply (insert prems seps ret_address)
    text \<open>Apply VCG/symb.\ execution\<close>

...
EightHigh | EightLow | Sixteen | ThirtyTwo | SixtyFour 
...
text {*
  A \emph{text section} of a binary is a section with a name and a list of instructions.
*}
record text_section =
  ts_name :: string
...
record data_section =
  ds_name :: string
  ds_size :: nat
  ds_data :: "data_value list"

text {*
  The deep embedding of an assembly file consists of a list of text- and datasections.
*}
record assembly = 
  text_sections :: "text_section list"
  data_sections :: "data_section list"  
  labels_to_addresses :: "(string \<times> (string \<times> nat)) list"
  labels_to_indices   :: "(string \<times> (string \<times> nat)) list"

record state =
  regs :: "register_mnemonic \<Rightarrow> 64 word"
  mem :: "64 word \<Rightarrow> 8 word"
  flags :: "flag \<Rightarrow> bool"
  pc :: int (* actually nat, but this is more readable *)
  ts :: int (* actually nat, but this is more readable *)
